...
function ChatWindowHeader({
  ...
  return (
    <div className="shrink-0 flex justify-between items-center gap-2 bg-gray-400 px-2 py-1 rounded-t-lg dark:bg-gray-800">
      ...
      <div className="flex gap-1 items-center">
        {chatId && !closed && (
          <Dropdown>
            ...
            >
              <DropdownItem key="close">Close chat</DropdownItem>
            </DropdownMenu>
          </Dropdown>
        )}
...
function LoadingAssistantResponse() {
  return (
    <div className="flex gap-2 items-end">
      ...
      <div className="bg-white rounded-none dark:bg-gray-800 text-gray-800 dark:text-white mr-[20%] rounded-bl-none p-2">
...
function AssistantMessage({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1 items-start">
      <div className="text-gray-800 dark:text-white text-xs pl-2">
        Assistant
      </div>
      <div className="bg-gray-200 rounded-none dark:bg-gray-800 text-gray-800 dark:text-white mr-[20%] rounded-bl-none p-2">
...
function UserMessage({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1 items-end">
      <div className="bg-gray-200 rounded-none dark:bg-gray-800 text-gray-800 dark:text-white ml-[20%] rounded-br-none p-2">
        {typeof children === "string" ? (
          <MarkdownContent content={children} />
        ) : (
          children
...
function ChatWindowMessages({
  ...
  return (
    <div className="flex flex-col grow p-2 gap-4 overflow-auto">
      <AssistantMessage>
        Hello! I&apos;m Rowboat, your personal assistant. How can I help you
        today?
      </AssistantMessage>
...
function ChatWindowInput({
  ...
  return (
    <div className="bg-white rounded-none dark:bg-gray-900 shrink-0 p-2">
      <Textarea
...
function ChatWindowBody({
  ...
  return (
    <div className="flex flex-col grow bg-white rounded-b-lg dark:bg-gray-900 overflow-auto">
      <ChatWindowMessages
        messages={messages}
        loadingAssistantResponse={loadingAssistantResponse}
      />
      {!closed && <ChatWindowInput handleUserMessage={handleUserMessage} />}
      {closed && (
        <div className="flex flex-col items-center py-4 gap-2">
          <div className="text-gray-800 dark:text-white">
            This chat is closed
          </div>
          <Button onPress={resetState}>Start new chat</Button>
        </div>
      )}
    </div>
  );
...
export function App({ apiUrl }: { apiUrl: string }) {
  ...
  const [minimized, setMinimized] = useState(
    searchParams.get("minimized") === "true",
  );
  ...
  const fetchLastChat = useCallback(async (): Promise<{
    ...
    const { chats }: z.infer<typeof apiV1.ApiGetChatsResponse> =
      await response.json();
    if (chats.length === 0) {
      return null;
    }
    ...
    do {
      ...
      const {
        messages,
        next,
      }: z.infer<typeof apiV1.ApiGetChatMessagesResponse> =
  ...
  async function createChat(): Promise<string> {
    ...
    const { id }: z.infer<typeof apiV1.ApiCreateChatResponse> =
      await response.json();
    setChatId(id);
    return id;
  }
  ...
  async function getAssistantResponse(
    ...
    const { content }: z.infer<typeof apiV1.ApiChatTurnResponse> =
      await response.json();
    return {
      role: "assistant",
      content: content || "",
