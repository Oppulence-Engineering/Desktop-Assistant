...
import { IHandleIntegrationWebhookRequestController } from "@/src/interface-adapters/controllers/integration/webhook/handle-integration-webhook-request.controller";
import { nanoid } from "nanoid";

const handleIntegrationWebhookRequestController = container.resolve<IHandleIntegrationWebhookRequestController>("handleIntegrationWebhookRequestController");

export async function POST(request: Request) {
    const id = nanoid();
    const logger = new PrefixLogger(`integration-webhook-[${id}]`);
    ...
    // handle webhook
    try {
        await handleIntegrationWebhookRequestController.execute({
            headers,
            payload,
        });
    } catch (error) {
        logger.log('Error handling integration webhook', error);
    }
