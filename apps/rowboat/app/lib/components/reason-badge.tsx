...
export function ReasonBadge({
  ...
  if (isJob && jobId && projectId) {
    return (
      <Link
        ...
        className={`inline-flex items-center px-2 py-1 rounded-none text-xs font-mono font-medium ${color} hover:opacity-80 transition-opacity`}
      >
        {label}
      </Link>
    );
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-1 rounded-none text-xs font-mono font-medium ${color}`}
    >
      {label}
    </span>
  );
