...
export async function getCustomerForUserId(
  userId: string,
): Promise<z.infer<typeof Customer> | null> {
...
export async function getBillingCustomer(id: string): Promise<z.infer<typeof Customer> | null> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch billing customer: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
...
async function createBillingCustomer(
  userId: string,
  email: string,
): Promise<z.infer<typeof Customer>> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to create billing customer: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
  const parseResult = Customer.safeParse(json);
...
export async function syncWithStripe(customerId: string): Promise<void> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to sync with stripe: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
}
...
export async function authorize(
  customerId: string,
  request: z.infer<typeof AuthorizeRequest>,
): Promise<z.infer<typeof AuthorizeResponse>> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to authorize billing: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
  ...
  if (!parseResult.success) {
    throw new Error(
      `Failed to parse authorize billing response: ${JSON.stringify(parseResult.error)}`,
    );
  }
...
export async function logUsage(customerId: string, request: z.infer<typeof LogUsageRequest>) {
  ...
  console.log(
    `[${reqId}] logging billing usage for customer ${customerId} to ${BILLING_API_URL}`,
    reqId,
  ...
  console.log(
    `[${reqId}] completed logging billing usage for customer ${customerId}`,
    reqId,
    response.status,
    response.statusText,
  );
  if (!response.ok) {
    throw new Error(
      `Failed to log usage: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
}
...
export async function getUsage(customerId: string): Promise<z.infer<typeof UsageResponse>> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get usage: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
...
export async function createCustomerPortalSession(
  ...
  const response = await fetch(
    `${BILLING_API_URL}/api/customers/${customerId}/customer-portal-session`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${BILLING_API_KEY}`,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get customer portal url: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
  ...
  if (!parseResult.success) {
    throw new Error(
      `Failed to parse customer portal session response: ${JSON.stringify(parseResult.error)}`,
    );
  }
...
export async function getPrices(): Promise<z.infer<typeof PricesResponse>> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get prices: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
...
export async function updateSubscriptionPlan(
  customerId: string,
  request: z.infer<typeof UpdateSubscriptionPlanRequest>,
): Promise<string> {
  const response = await fetch(
    `${BILLING_API_URL}/api/customers/${customerId}/update-sub-session`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${BILLING_API_KEY}`,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to update subscription plan: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
  ...
  if (!parseResult.success) {
    throw new Error(
      `Failed to parse update subscription plan response: ${JSON.stringify(parseResult.error)}`,
    );
  }
  return parseResult.data.url;
}
...
/**
 * Cached read of the per-customer model catalog for prerender-friendly pages.
 * The catalog is read-only from the app's perspective and changes only on
 * plan changes, so a minutes-scale lifetime self-heals staleness; the tag
 * allows targeted invalidation if a plan-change flow ever needs it.
 */
export async function getEligibleModelsCached(
  customerId: string,
): Promise<z.infer<typeof ModelsResponse>> {
  "use cache";
  cacheLife("minutes");
  cacheTag(`billing-models-${customerId}`);
  return getEligibleModels(customerId);
}
...
export async function getEligibleModels(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to get eligible models: ${response.status} ${response.statusText} ${await response.text()}`,
    );
  }
  const json = await response.json();
  ...
  if (!parseResult.success) {
    throw new Error(
      `Failed to parse eligible models response: ${JSON.stringify(parseResult.error)}`,
    );
  }
...
export async function requireActiveBillingSubscription(): Promise<z.infer<typeof Customer>> {
  ...
  if (
    USE_BILLING &&
    billingCustomer.subscriptionStatus !== "active" &&
    billingCustomer.subscriptionStatus !== "past_due"
  ) {
    redirect("/billing");
  }
