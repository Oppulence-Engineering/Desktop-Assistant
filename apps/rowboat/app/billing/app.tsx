...
export function BillingPage({ customer, usage }: BillingPageProps) {
  ...
  const usedCredits = Math.ceil(
    (usage.sanctionedCredits - usage.availableCredits) / 10 ** 6,
  );
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-8 space-y-8">
            ...
            <div className="flex flex-col items-end gap-2 min-w-[200px]">
              {(plan === "free" || plan === "starter") && (
                <Button
                  variant="primary"
                  size="lg"
                  className="bg-gradient-to-r from-indigo-200 via-purple-200 to-pink-100 text-indigo-700 dark:text-indigo-200 shadow hover:from-indigo-300 hover:to-pink-200 rounded-none border border-indigo-200 dark:border-indigo-700"
      ...
      <section className="card">
        <div className="px-4 pt-4 pb-6">
          <SectionHeading>Credits Overview</SectionHeading>
        </div>
        <HorizontalDivider />
        <div className="p-6 space-y-6">
          ...
          {usage.availableCredits < 0 && (
            <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-none">
              ...
              >
                ⚠️ You have exceeded your credit limit. Please upgrade your plan
                or contact support to avoid service interruptions.
              </p>
            </div>
          )}
          ...
          {usage.availableCredits >= 0 &&
            (usage.sanctionedCredits - usage.availableCredits) /
              usage.sanctionedCredits >
              0.8 && (
              <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-none">
                ...
                >
                  ⚠️ You have used more than 80% of your credits. Consider
      ...
      <section className="card">
        <div className="px-4 pt-4 pb-6">
          <SectionHeading>Usage split</SectionHeading>
        </div>
        <HorizontalDivider />
        <div className="p-6 space-y-6">
          {usageData.length === 0 ? (
            ...
            usageData.map(({ type, credits, totalUsedCredits }) => {
              const percentage =
                totalUsedCredits > 0 ? (credits / totalUsedCredits) * 100 : 0;
