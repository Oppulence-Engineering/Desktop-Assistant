...
const valueTextareaStyles =
  "rounded-none p-3 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-750 focus:shadow-inner focus:ring-2 focus:ring-indigo-500/20 dark:focus:ring-indigo-400/20 placeholder:text-black dark:placeholder:text-white";
...
export function PromptConfig({
  ...
  const atMentions = [
    ...
    ...prompts
      ...
      .map((p) => ({ id: `prompt:${p.name}`, value: `prompt:${p.name}` })),
    ...tools.map((tool) => ({
      id: `tool:${tool.name}`,
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 p-4">
        ...
        {prompt.type === "base_prompt" && (
          <div className="space-y-4">
            <div className="space-y-2">
              <label className={sectionHeaderStyles}>Name</label>
              <div
                className={clsx(
        ...
        <div className="space-y-4">
          <label className={sectionHeaderStyles}>Value</label>
          <Textarea
            value={prompt.prompt}
            onChange={(e) => {
              handleUpdate({
