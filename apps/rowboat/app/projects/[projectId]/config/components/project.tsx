...
import {
  getToolkit,
  listIntegrationTriggerDeployments,
  deleteIntegrationTriggerDeployment,
  ...
  return (
    <div className="rounded-none border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
      <div className="px-6 pt-4">
        ...
        {description && <p className={sectionDescriptionStyles}>{description}</p>}
      </div>
...
export function LeftLabel({ label }: { label: string }) {
  return <Label label={label} />;
}

export function RightContent({ children }: { children: React.ReactNode }) {
...
function ProjectNameSection({
  ...
  return (
    <Section title="Project Name" description="The name of your project.">
...
function ProjectIdSection({ projectId }: { projectId: string }) {
  return (
    <Section title="Project ID" description="Your project's unique identifier.">
...
function SecretSection({ projectId }: { projectId: string }) {
  ...
  const formattedSecret = hidden
    ? `${secret?.slice(0, 2)}${"•".repeat(5)}${secret?.slice(-2)}`
    : secret;
  ...
  const handleRotateSecret = async () => {
    if (
      !confirm(
        "Are you sure you want to rotate the secret? All existing signatures will become invalid.",
      )
    ) {
      return;
    }
          ...
          <div className="flex flex-row gap-4 items-center">
            ...
            <div className="flex flex-row gap-4 items-center">
              ...
              <CopyButton
                ...
                successLabel="Copied"
              />
              <Button size="sm" variant="primary" onClick={handleRotateSecret} disabled={loading}>
                Rotate
              </Button>
            </div>
          </div>
...
function ApiKeyDisplay({ apiKey, onDelete }: { apiKey: string; onDelete: () => void }) {
  ...
  const formattedKey = isVisible
    ? apiKey
    : `${apiKey.slice(0, 2)}${"•".repeat(5)}${apiKey.slice(-2)}`;
...
function ApiKeysSection({ projectId }: { projectId: string }) {
  ...
  return (
    <Section
      title="API Keys"
      description="API keys are used to authenticate requests to the Oppulence API."
    >
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded-none overflow-hidden">
            ...
            {keys.map((key) => (
              <div
                key={key.id}
                className="grid grid-cols-12 items-center border-b border-gray-200 dark:border-gray-700 last:border-0 p-4"
              >
                <div className="col-span-7">
                  <ApiKeyDisplay apiKey={key.key} onDelete={() => handleDeleteKey(key.id)} />
                </div>
...
function DisconnectToolkitsSection({
  projectId,
  onProjectConfigUpdated,
}: {
  projectId: string;
  ...
  const loadConnectedToolkits = useCallback(async () => {
    ...
    try {
      ...
      const toolkits = (await Promise.all(toolkitPromises)).filter(Boolean) as (
        | ConnectedToolkit
        | (ConnectedToolkit & { connectedAccount: null })
      )[];
  ...
  const handleConfirmDisconnect = async () => {
    ...
    try {
      ...
      if (currentWorkflow) {
        ...
        const updatedTools = currentWorkflow.tools.filter(
          (tool) =>
      ...
      const toolkitTriggers = triggers.items.filter(
        (trigger) => trigger.toolkitSlug === selectedToolkit.slug,
      );
      ...
      if (selectedToolkit.connectedAccount) {
        await deleteConnectedAccount(projectId, selectedToolkit.slug);
      }
...
function DeleteProjectSection({ projectId }: { projectId: string }) {
  ...
  return (
    <Section title="Delete Project" description="Permanently delete this project and all its data.">
      <div className="space-y-4">
        <div className="p-4 bg-red-50/10 dark:bg-red-900/10 rounded-none">
          <p className="text-sm text-red-700 dark:text-red-300">
            Deleting a project will permanently remove all associated data, including workflows,
            sources, and API keys. This action cannot be undone.
          </p>
        </div>

        <Button variant="primary" size="sm" onClick={onOpen} disabled={loadingInitial} color="red">
          Delete project
        </Button>
        ...
        <Modal isOpen={isOpen} onClose={onClose}>
          <ModalContent>
            ...
            <ModalFooter>
              <Button variant="secondary" onClick={onClose} disabled={deletingProject}>
                Cancel
              </Button>
              <Button
                variant="primary"
