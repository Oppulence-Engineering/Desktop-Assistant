...
import { listToolkits } from "@/app/actions/integration.actions";
...
type ToolkitListResponse = z.infer<
  ReturnType<typeof ZListResponse<typeof ZToolkit>>
>;
type ProjectType = z.infer<typeof Project>;

interface SelectIntegrationToolkitProps {
  projectId: string;
...
export function SelectIntegrationToolkit({
  projectId,
  tools,
  onSelectToolkit,
  initialToolkitSlug,
  ...
  const loadAllToolkits = useCallback(async () => {
    ...
    try {
      ...
      do {
        const response: ToolkitListResponse = await listToolkits(
          projectId,
          cursor,
        );
      ...
      if (filterByTriggers) {
        finalToolkits = finalToolkits.filter(
          (toolkit) => toolkit.meta.triggers_count > 0,
        );
      }
      if (filterByTools) {
        finalToolkits = finalToolkits.filter(
          (toolkit) => toolkit.meta.tools_count > 0,
        );
      }
      ...
      setError(
        "Unable to load all Integration toolkits. Please check your connection and try again.",
      );
  ...
  const handleSelectToolkit = useCallback(
    (toolkit: ToolkitType) => {
  ...
  const filteredToolkits = toolkits
    ...
    .sort((a, b) => {
      ...
      const aConnected =
        !a.no_auth &&
        projectConfig?.integrationConnectedAccounts?.[a.slug]?.status === "ACTIVE";
      const bConnected =
        !b.no_auth &&
        projectConfig?.integrationConnectedAccounts?.[b.slug]?.status === "ACTIVE";
    ...
    <div className="space-y-6">
          ...
          <div className="flex-1 flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-4 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-none
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
              ...
              {filteredToolkits.length === 1 ? "toolkit" : "toolkits"}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredToolkits.map((toolkit) => {
          const isConnected =
            toolkit.no_auth ||
            projectConfig?.integrationConnectedAccounts?.[toolkit.slug]?.status ===
              "ACTIVE";
