...
export function SelectComposioToolkit({
  ...
  const loadAllToolkits = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Unable to load all Composio toolkits. Please check your connection and try again.",
      );
  ...
  const filteredToolkits = toolkits
    ...
    .sort((a, b) => {
      ...
      const aConnected =
        !a.no_auth &&
        projectConfig?.composioConnectedAccounts?.[a.slug]?.status === "ACTIVE";
      const bConnected =
        !b.no_auth &&
        projectConfig?.composioConnectedAccounts?.[b.slug]?.status === "ACTIVE";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredToolkits.map((toolkit) => {
          const isConnected =
            toolkit.no_auth ||
            projectConfig?.composioConnectedAccounts?.[toolkit.slug]?.status ===
              "ACTIVE";
