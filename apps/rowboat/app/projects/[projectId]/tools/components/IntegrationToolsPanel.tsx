...
import { listTools } from "@/app/actions/integration.actions";
import { z } from "zod";
...
export function IntegrationToolsPanel({
  ...
  const selectedToolSlugs = workflowTools
    ...
    .map((tool) => tool.integrationData!.slug);

  // Filter out already selected tools
  const availableTools = tools.filter(
    (tool) => !selectedToolSlugs.includes(tool.slug),
  );
  ...
  const loadToolsForToolkit = useCallback(
    async (
      toolkitSlug: string,
      ...
      try {
        ...
        const response: ToolListResponse = await listTools(
          projectId,
          toolkitSlug,
          search,
          cursor,
  ...
  const handleToolSelectionChange = useCallback(
    (toolSlug: string, selected: boolean) => {
  ...
  const handleAddSelectedTools = useCallback(() => {
    ...
    const selectedToolObjects = tools.filter((tool) =>
                  ...
                  <div className="flex items-start gap-3">
                    <Checkbox
                      ...
                      onValueChange={(selected) =>
                        handleToolSelectionChange(tool.slug, selected)
                      }
                      size="sm"
                    />
                    <div className="flex-1 text-left flex flex-col gap-1">
                      ...
                      <div
                        className="font-mono text-xs text-gray-500 dark:text-gray-400 text-left truncate max-w-[300px] bg-gray-100 dark:bg-gray-700 p-1 rounded-none"
                        title={tool.slug}
                      >
                        {tool.slug}
                      </div>
