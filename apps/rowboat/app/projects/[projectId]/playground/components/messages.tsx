...
function UserMessage({ content }: { content: string }) {
  return (
    <div className="self-end flex flex-col items-end gap-1 mt-5 mb-8">
      <div className="text-gray-500 dark:text-gray-400 text-xs">User</div>
      <div className="max-w-[85%] inline-block">
        <div
          className="bg-blue-100 dark:bg-blue-900/40 px-4 py-2.5
                    rounded-none rounded-br-lg text-sm leading-relaxed
      ...
      <div className="max-w-[85%] inline-block">
        <div className="text-gray-500 dark:text-gray-400 text-xs pl-1 flex justify-between items-center mb-2">
          ...
          {(Boolean(
            ...
            <MessageActionsMenu
              ...
              onExplain={
                onExplain
                  ? () => onExplain("assistant", content, index)
                  : () => {}
              }
              onJson={() => setJsonMode(!jsonMode)}
              jsonLabel={
                jsonMode ? "View formatted content" : "View complete JSON"
              }
            />
          )}
        </div>
        <div
          className="bg-purple-50 dark:bg-purple-900/30 px-4 py-2.5
                    rounded-none rounded-bl-lg text-sm leading-relaxed
        ...
        >
          <div className="text-left mb-2">
            ...
            {isJsonContent && jsonMode ? (
              <pre
                className={`text-xs leading-snug bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-200 rounded-none px-2 py-1 font-mono shadow-sm border border-zinc-100 dark:border-zinc-700 ${
                  wrapText
                ...
                style={{
                  fontFamily:
                    "'JetBrains Mono', 'Fira Mono', 'Menlo', 'Consolas', 'Liberation Mono', monospace",
                }}
              >
                {formattedJson}
              </pre>
          ...
          <div className="flex justify-end items-center gap-6 mt-2">
            <div className="text-right text-xs">{deltaDisplay}</div>
          </div>
        </div>
      </div>
    </div>
              ...
              <div className="flex flex-wrap gap-3">
                {imagePreviews.map((img, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="group relative rounded-none p-2 bg-white dark:bg-zinc-900"
                    >
                      <a
                        href={src}
                        download={filename}
                        className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 dark:bg-zinc-900/80 rounded-none p-1 shadow hover:bg-white dark:hover:bg-zinc-800"
...
function ToolCall({
  ...
  for (let i = parentIndex; i < messages.length; i++) {
    ...
    if (m.role === "tool" && m.toolCallId === toolCall.id) {
      nearestResult = m as any;
      break;
    }
  }
...
function TransferToAgentToolCall({
  ...
  const typedResult = availableResult
    ? (JSON.parse(availableResult.content) as { assistant: string })
    : undefined;
  if (!typedResult) {
    return <></>;
  }
            ...
            onExplain={
              onExplain
                ? () =>
                    onExplain(
                      "transition",
                      `From: ${sender} To: ${typedResult.assistant}`,
                      parentIndex,
                    )
                : () => {}
            }
...
function ClientToolCall({
  ...
  if (availableResult && typeof availableResult.content === "string") {
    try {
      ...
      const imgs = Array.isArray(parsedResult?.images)
        ? parsedResult.images
        : [];
      imagePreviews = imgs
        .filter(
        ...
        .map((img: any) => ({
          ...
          dataBase64:
            typeof img?.dataBase64 === "string" ? img.dataBase64 : undefined,
  ...
  if (isCompressed) {
    return (
      <div className="self-start flex flex-col gap-1 my-5">
        {(Boolean(showDebugMessages && typeof onFix === "function") ||
          Boolean(showDebugMessages && typeof onExplain === "function")) && (
          <div className="text-gray-500 dark:text-gray-400 text-xs pl-1 flex justify-between items-center">
            <span>{sender}</span>
            <MessageActionsMenu
              showFix={Boolean(
                showDebugMessages && typeof onFix === "function",
              )}
              showExplain={Boolean(
                showDebugMessages && typeof onExplain === "function",
              )}
              showJson={false}
              onFix={
                onFix
                  ? () =>
                      onFix(`Tool call: ${toolCall.function.name}`, parentIndex)
                  : () => {}
              }
              onExplain={
                onExplain
        ...
        <div className="min-w-[85%]">
          <div
            className="border border-gray-200 dark:border-gray-700 p-3
                        rounded-none rounded-bl-lg flex flex-col gap-2
        ...
        >
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            {availableResult && (
              <div
                className={
                  (paramsExpanded ? "mt-4 " : "") + "flex flex-col gap-3 w-full"
                }
              >
                {imagePreviews.length > 0 && (
                  <div className="flex flex-wrap gap-3">
                    {imagePreviews.map((img, i) => (
                      <div
                        key={i}
                        className="rounded-none border border-gray-200 dark:border-gray-700 p-2 bg-white dark:bg-zinc-900"
                      >
                        <Image
                          src={
                            img.url
...
function ExpandableContent({
  ...
  return (
    <div className="flex flex-col gap-2 min-w-0">
      <div
        className="flex gap-1 items-start cursor-pointer text-gray-500 dark:text-gray-400 min-w-0"
...
export function Messages({
  ...
  onFix?: (message: string, index: number) => void;
  onExplain?: (
    type: "assistant" | "tool" | "transition",
    message: string,
    index: number,
  ) => void;
}) {
  ...
  const renderMessage = (message: z.infer<typeof Message>, index: number) => {
    const isFirstAssistant =
      message.role === "assistant" && index === firstAssistantIdx;
    if (message.role === "assistant") {
      ...
      const agentConfig = workflow.agents.find(
        (a) => a.name === message.agentName,
      );
      const isInternalOrPipeline =
        message.responseType === "internal" ||
        (agentConfig &&
          (agentConfig.outputVisibility === "internal" ||
            agentConfig.type === "pipeline"));
  ...
  const isAgentTransition = (message: z.infer<typeof Message>) => {
    return (
      ...
      message.toolCalls.some((tc) =>
        tc.function.name.startsWith("transfer_to_"),
      )
    );
  };
...
const iconWithLabelClass =
  "group relative flex items-center gap-1 text-xs cursor-pointer hover:underline";
const iconLabelClass =
  "absolute left-full ml-2 px-2 py-1 rounded bg-zinc-800 text-white text-xs opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-10";
