...
import { deleteIntegrationTriggerDeployment, fetchIntegrationTriggerDeployment } from "@/app/actions/integration.actions";
...
export function IntegrationTriggerDeploymentView({ projectId, deploymentId }: { projectId: string; deploymentId: string; }) {
    const [deployment, setDeployment] = useState<z.infer<typeof IntegrationTriggerDeployment> | null>(null);
    const [loading, setLoading] = useState<boolean>(true);
    const [deleting, setDeleting] = useState(false);
    const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

    const jobsFilters = useMemo(() => ({ integrationTriggerDeploymentId: deploymentId } satisfies z.infer<typeof JobFiltersSchema>), [deploymentId]);
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            try {
                const res = await fetchIntegrationTriggerDeployment({ deploymentId });
                if (ignore) return;
                setDeployment(res);
            } finally {
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            await deleteIntegrationTriggerDeployment({ projectId, deploymentId: deployment.id });
            ...
            {showDeleteConfirm && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white dark:bg-gray-800 rounded-none p-6 max-w-md mx-4">
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">Are you sure you want to delete this external trigger? This will remove the linked webhook in Integration and delete this deployment.</p>
