...
export function Action({
  ...
  const isExternalTriggerCreate =
    action.config_type === "external_trigger" && action.action === "create_new";
  ...
  const appliedFields = Object.keys(action.config_changes).filter(
    (key) => appliedChanges[getAppliedChangeKey(msgIndex, actionIndex, key)],
  );
  let allApplied =
    externallyApplied ||
    Object.keys(action.config_changes).every((key) =>
      appliedFields.includes(key),
    );
  if (
    !externallyApplied &&
    (action.action === "delete" || action.config_type === "start_agent")
  ) {
    allApplied = false;
  }
  ...
  const handleFieldChange = (field: string) => {
    ...
    setAppliedChanges((prev) => {
      ...
      const allFieldsApplied = Object.keys(action.config_changes).every(
        (key) => newApplied[getAppliedChangeKey(msgIndex, actionIndex, key)],
      );
  ...
  const handleApplyAll = () => {
    ...
    const appliedKeys = Object.keys(action.config_changes).reduce(
      (acc, key) => {
  ...
  function getMainDiffField() {
    if (
      ...
      "instructions" in action.config_changes
    )
      return "instructions";
    if (action.config_type === "tool" && "description" in action.config_changes)
      return "description";
    if (action.config_type === "prompt" && "prompt" in action.config_changes)
      return "prompt";
  ...
  return (
    <div
      className={clsx(
        "flex flex-col rounded-none border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-xs",
        "transition-shadow duration-150",
        {
          "border-l-2 border-l-blue-500":
            !stale && !allApplied && action.action == "create_new",
          "border-l-2 border-l-yellow-500":
            !stale && !allApplied && action.action == "edit",
          "border-l-2 border-l-red-500":
            !stale && !allApplied && action.action == "delete",
        ...
        <div className="flex items-center gap-2 px-2 py-1 border-b border-zinc-100 dark:border-zinc-800">
          ...
          >
            {action.config_type === "tool" && toolkitLogo ? (
              ...
              "🧑‍💼"
            ) : action.config_type === "tool" ? (
              "🛠️"
            ) : action.config_type === "pipeline" ? (
              "⚙️"
            ) : action.config_type === "start_agent" ? (
              "🏁"
            ) : action.config_type === "prompt" ? (
              "💬"
            ) : action.config_type === "one_time_trigger" ? (
              "⏰"
            ) : action.config_type === "recurring_trigger" ? (
              "🔄"
            ) : action.config_type === "external_trigger" ? (
              "🔗"
            ) : (
              "💬"
            )}
          ...
          <span className="font-semibold text-sm text-zinc-800 dark:text-zinc-100 truncate flex-1">
            {action.action === "create_new"
              ...
              : action.action === "edit"
                ? "Edit"
                : "Delete"}{" "}
            {action.config_type}: {action.name}
          </span>
          ...
          <div className="flex items-center gap-1">
            ...
            >
              ...
              <span>
                {allApplied
                  ? "Applied"
                  : isExternalTriggerCreate
                    ? "Open setup"
                    : "Apply"}
              </span>
            </button>
            {action.action !== "delete" && !isExternalTriggerCreate && (
              <button
...
export function ActionSummary() {
  const { msgIndex, actionIndex, action, workflow, appliedFields, stale } =
    useContext(ActionContext);
  if (!action || !workflow) return null;

  return (
    <div className="px-1 my-1">
      <div className="bg-white dark:bg-gray-800 rounded-none p-2 text-sm">
...
export function ActionField({
  ...
  const markdownPreviewCondition =
    (action.config_type === "agent" && field === "instructions") ||
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-800 rounded-none">
      <div className="flex justify-between items-start">
        <div className="text-xs font-semibold px-2 py-1 text-gray-600 dark:text-gray-300">
          {field}
        </div>
        {previewCondition && (
          <div className="flex gap-4 items-center bg-gray-50 dark:bg-gray-700 rounded-bl-sm rounded-tr-sm px-2 py-1">
            <button
            ...
            {action.action === "edit" && !action.error && (
              <button
                className={clsx(
...
export function StreamingAction({
  ...
  const [loadingStage, setLoadingStage] = useState<"fetching" | "configuring">(
    "fetching",
  );
