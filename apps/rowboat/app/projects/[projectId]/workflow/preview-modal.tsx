...
function PreviewModal({
  ...
  return (
    <div className="fixed left-0 top-0 w-full h-full bg-gray-500/50 backdrop-blur-sm flex justify-center items-center z-50">
      <div className="relative bg-gradient-to-br from-white to-zinc-50 dark:from-zinc-900 dark:to-zinc-800 rounded-none shadow-2xl p-6 w-[98vw] max-w-7xl max-h-[90vh] flex flex-col gap-6">
        {/* Close button */}
        <button
          className="absolute top-4 right-4 rounded-full p-2 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
          onClick={onClose}
        >
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-none grow overflow-auto border border-zinc-100 dark:border-zinc-800">
          <div className="h-full flex flex-col overflow-auto">
            {view === "preview" && (
              <div className="flex gap-1 overflow-auto text-sm">
                {oldValue !== undefined && (
                  <ReactDiffViewer
                    oldValue={oldValue}
                    newValue={newValue}
                ...
                {oldValue === undefined && (
                  <pre className="p-2 overflow-auto">{newValue}</pre>
                )}
              </div>
            )}
            {view === "markdown" && (
              <div className="flex gap-1">
                {oldValue !== undefined && (
                  <div className="w-1/2 flex flex-col border-r-2 border-gray-200 dark:border-zinc-800 overflow-auto">
                    <div className="text-gray-800 dark:text-gray-200 font-semibold italic text-sm px-2 py-1 border-b border-gray-200 dark:border-zinc-800">
                      Old
                    </div>
                    <div className="p-2 overflow-auto">
                      <MarkdownContent content={oldValue} />
                    </div>
                  </div>
                )}
                <div
                ...
                >
                  ...
                  <div className="p-2 overflow-auto">
                    <MarkdownContent content={newValue} />
                  </div>
                </div>
              </div>
            )}
