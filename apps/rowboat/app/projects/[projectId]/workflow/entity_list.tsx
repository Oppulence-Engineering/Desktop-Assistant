...
// Common classes
const headerClasses =
  "font-semibold text-zinc-700 dark:text-zinc-300 flex items-center justify-between w-full";
const buttonClasses =
  "text-sm px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:hover:bg-indigo-900 dark:text-indigo-400";
...
const ListItemWithMenu = ({
  ...
  return (
    <div
      className={clsx(
        "group flex items-center gap-2 px-3 py-2 rounded-none min-h-[24px] cursor-pointer",
        {
    ...
    >
      ...
      <div
        ...
        className={clsx("flex-1 flex items-center gap-2 text-sm text-left", {
      ...
      >
        <div
          className={clsx(
            "shrink-0 flex items-center justify-center w-3 h-3",
            iconClassName,
          )}
        >
          {mcpServerName ? (
            <ServerLogo
              ...
              className="h-3 w-3"
              fallback={
                <ImportIcon className="w-3 h-3 text-blue-600 dark:text-blue-500" />
              }
            />
          ) : (
            icon
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <div className="opacity-100">{menuContent}</div>
      </div>
    </div>
  );
};
...
const ServerCard = ({
  ...
  return (
    <div className="mb-1 group">
      <div className="flex items-center gap-2 px-2 py-1 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded-none transition-colors">
        <button
    ...
    <div className="mb-1 group">
      <div className="flex items-center gap-2 px-2 py-1 hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded-none transition-colors">
        ...
        <button
          ...
          className={`w-4 h-4 flex items-center justify-center transition-opacity rounded ${
            pipelineAgents.length > 0
              ? "group-hover:opacity-100 opacity-60 hover:bg-gray-200 dark:hover:bg-gray-700"
        ...
        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
          <Dropdown>
            <DropdownTrigger>
              <button className="p-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-none transition-colors">
            ...
            >
              {startAgentName !== pipeline.name ? (
                <>
                  <DropdownItem key="set-main-agent">
                    Set as start agent
                  </DropdownItem>
        ...
        <div className="ml-6 mt-0.5 space-y-0.5 border-l border-gray-200 dark:border-gray-700 pl-3">
          {pipelineAgents.map((agent, index) => (
            <div key={`pipeline-agent-${index}`} className="group/agent">
              <div
                className={clsx(
                  "flex items-center gap-2 px-3 py-2 rounded-none min-h-[24px] cursor-pointer",
                  {
                    "bg-indigo-50 dark:bg-indigo-950/30":
                      selectedEntity?.type === "agent" &&
                      selectedEntity.name === agent.name,
              ...
              >
                ...
                <div className="opacity-0 group-hover/agent:opacity-100 transition-opacity">
                  <button
                    className="p-1 hover:bg-red-100 dark:hover:bg-red-900 rounded-none transition-colors"
                    onClick={(e) => {
                      e.stopPropagation();
...
>(function EntityList(
  ...
  const getPanelSize = (panelName: "agents" | "tools" | "data" | "prompts") => {
    ...
    if (
      expandedVisiblePanels.length > 0 &&
      expandedVisiblePanels[0] === panelName
    ) {
      size += availableSpace;
    }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      const isPipelineDrag = pipelines.some(
        (pipeline) => pipeline.name === active.id,
      );
      const isPipelineTarget = pipelines.some(
        (pipeline) => pipeline.name === over.id,
      );
      ...
      if (isPipelineDrag && isPipelineTarget) {
        ...
        const newIndex = pipelines.findIndex(
          (pipeline) => pipeline.name === over.id,
        );
      ...
      >
        ...
        >
          <Panel
            ...
            title={
              <div
                className={`${headerClasses} rounded-none transition-colors h-full`}
              >
                    ...
                    <div className="space-y-1">
                      ...
                      {(() => {
                        ...
                        const individualAgents = agents.filter(
                          (agent) =>
                            !pipelineAgentNames.has(agent.name) &&
                            agent.type !== "pipeline",
                        );
                ...
                <div className="p-2">
                  {(() => {
                    ...
                    for (const t of defaults)
                      if (!toolMap.has(t.name)) toolMap.set(t.name, t as any);
                      ...
                      return (
                        <div className="space-y-1">
                          ...
                          {(() => {
                            ...
                            const serverTools = toolsForDisplay.reduce(
                              (acc, tool) => {
                                  ...
                                  <div className="mt-2">
                                    {customTools.map((tool, index) => (
                                      ...
                                      >
                                        ...
                                        {tool.mockTool && (
                                          <span className="ml-2 px-1 py-0 rounded bg-purple-50 text-purple-400 dark:bg-purple-900/40 dark:text-purple-200 text-[11px] font-normal align-middle">
                                            Mocked
                                          </span>
                                        )}
                                        {!tool.isLibrary && (
                  ...
                  {dataSources.length > 0 ? (
                    <div className="space-y-1">
                      {dataSources.map((dataSource, index) => {
                        ...
                        const isActive =
                          dataSource.active && dataSource.status === "ready";
                        ...
                        if (isPending) {
                          ...
                          statusPill = (
                            <Tooltip
                              content={dataSource.error || "Error"}
                              size="sm"
                              delay={500}
                            >
                          ...
                          >
                            <div
                              className={clsx(
                                ...
                                {
                                  "bg-indigo-50 dark:bg-indigo-950/30":
                                    selectedEntity?.type === "datasource" &&
                                    selectedEntity.name === dataSource.id,
                                  "hover:bg-zinc-50 dark:hover:bg-zinc-800": !(
                                    selectedEntity?.type === "datasource" &&
                                    selectedEntity.name === dataSource.id
                                  ),
                              ...
                              onClick={() =>
                                handleSelectDataSource(dataSource.id)
                              }
                            >
                              <div
                                ref={
                              ...
                              >
                                <div className="shrink-0 flex items-center justify-center w-3 h-3">
                                  <DataSourceIcon
                                    type={
                                      dataSource.data.type === "files_local" ||
                                      dataSource.data.type === "files_s3"
                                        ? "files"
        ...
        {SHOW_PROMPTS_SECTION && (
          ...
          >
            <Panel
              ...
              className={clsx("h-full", !expandedPanels.prompts && "h-[61px]!")}
              title={
                <div
                  className={`${headerClasses} rounded-none transition-colors h-full`}
                >
                        ...
                        {prompts.map((prompt, index) => (
                          <ListItemWithMenu
                            ...
                            isSelected={
                              ...
                              selectedEntity.name === prompt.name
                            }
                            ...
                            icon={
                              <ScrollText className="w-4 h-4 text-blue-600/70 dark:text-blue-500/70" />
                            }
                            menuContent={
                              <EntityDropdown
                                name={prompt.name}
      ...
      <AddVariableModal
        ...
        onConfirm={(name, value) => {
          if (editingVariable) {
            ...
            onUpdatePromptFromModal(editingVariable.name, {
              name,
              prompt: value,
            });
          } else {
...
const ComposioCard = ({
  ...
  const statusDot = (
    <Tooltip
      content={isToolkitConnected ? "Connected" : "Disconnected"}
      size="sm"
      delay={500}
    >
      <Circle
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      <ListItemWithMenu
        ...
        icon={
          <Component className="w-4 h-4 text-blue-600/70 dark:text-blue-500/70" />
        }
        dragHandle={
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <button
                ...
                className={clsx(
                  "relative group p-4 rounded-none border-2 flex flex-col items-start transition-all duration-200 text-left shadow-sm focus:outline-none",
              ...
              >
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-400 leading-snug mt-0 list-disc pl-4 space-y-0.5">
                  <li>
                    Perform specific internal tasks, such as parts of workflows,
                    pipelines, and data processing
                  </li>
                  <li>Cannot put out user-facing responses directly</li>
                  <li>
                    Can call other agents (both conversation and task agents)
                  </li>
                </ul>
              </button>
              ...
              >
                ...
                <ul className="text-sm text-gray-600 dark:text-gray-400 leading-snug mt-0 list-disc pl-4 space-y-0.5">
                  ...
                  <li>
                    Ideal for specific roles in customer support, chat
                    interfaces, and other end-user interactions
                  </li>
                  <li>
                    Can call other agents (both conversation and task agents)
                  </li>
        ...
        <ModalBody className="space-y-4">
          <div>
            ...
            <input
              ...
              className={clsx(
                "w-full px-3 py-2 border rounded-none text-sm",
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              className={clsx(
                "w-full px-3 py-2 border rounded-none text-sm resize-none",
                "focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500",
                "dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100",
                errors.value
                  ? "border-red-500"
