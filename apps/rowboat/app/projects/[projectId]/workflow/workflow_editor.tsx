  ...
  switch (action.type) {
    ...
    case "set_saving": {
      newState = produce(state, (draft) => {
        ...
        draft.present.lastUpdatedAt = !action.saving
          ? new Date().toISOString()
          : state.present.workflow.lastUpdatedAt;
      });
      break;
    }
    case "reorder_agents": {
      ...
      return {
        ...
        inversePatches: [
          ...state.inversePatches.slice(0, state.currentIndex),
          inversePatches,
        ],
        ...
        (draft) => {
          switch (action.type) {
            ...
            case "add_agent": {
              ...
              if (
                draft.workflow?.agents.some(
                  (agent) => agent.name === newAgentName,
                )
              ) {
                newAgentName = `New agent ${
                  draft.workflow.agents.filter((agent) =>
                    agent.name.startsWith("New agent"),
                  ).length + 1
                }`;
              }
              ...
              if (
                !draft.workflow?.startAgent ||
                draft.workflow.agents.length === 1
              ) {
            ...
            case "add_tool": {
              ...
              if (
                draft.workflow?.tools.some((tool) => tool.name === newToolName)
              ) {
                newToolName = `new_tool_${
                  draft.workflow.tools.filter((tool) =>
                    tool.name.startsWith("new_tool"),
                  ).length + 1
                }`;
              }
            ...
            case "add_prompt": {
              ...
              if (
                draft.workflow?.prompts.some(
                  (prompt) => prompt.name === newPromptName,
                )
              ) {
                newPromptName = `New Variable ${
                  draft.workflow?.prompts.filter((prompt) =>
                    prompt.name.startsWith("New Variable"),
                  ).length + 1
                }`;
              }
            ...
            case "add_pipeline": {
              ...
              if (pipelineAgents.length === 0) {
                ...
                for (const agentName of pipelineAgents) {
                  const existingAgent = draft.workflow.agents.find(
                    (a) => a.name === agentName,
                  );
                  if (!existingAgent) {
            ...
            case "delete_agent":
              ...
              if (draft.workflow.pipelines) {
                draft.workflow.pipelines = draft.workflow.pipelines.map(
                  (pipeline) => ({
                    ...pipeline,
                    agents: pipeline.agents.filter(
                      (agentName) => agentName !== action.name,
                    ),
                  }),
                );
              }
              ...
              if (draft.workflow.startAgent === action.name) {
                ...
                draft.workflow.startAgent =
                  draft.workflow.agents.length > 0
            ...
            case "delete_pipeline":
              if (draft.workflow.pipelines) {
                ...
                if (pipelineToDelete) {
                  ...
                  const startAgentBeingDeleted = agentsToDelete.includes(
                    draft.workflow.startAgent,
                  );
                  ...
                  agentsToDelete.forEach((agentName) => {
                    draft.workflow.agents = draft.workflow.agents.map(
                      (agent) => ({
                        ...agent,
                        instructions: agent.instructions.replace(
                          new RegExp(
                            `\\[@agent:${agentName}\\]\\(#mention\\)`,
                            "g",
                          ),
                          "",
                        ),
                    ...
                    // Update references in prompts
                    draft.workflow.prompts = draft.workflow.prompts.map(
                      (prompt) => ({
                        ...prompt,
                        prompt: prompt.prompt.replace(
                          new RegExp(
            ...
            case "update_agent": {
              ...
              if (action.agent.name && action.agent.name !== action.name) {
                ...
                if (
                  draft.selection?.type === "agent" &&
                  draft.selection.name === action.name
                ) {
                  draft.selection = {
                    type: "agent",
...
export function useEntitySelection() {
  ...
  if (!ctx)
    throw new Error(
      "useEntitySelection must be used within EntitySelectionContext",
    );
  return ctx;
}
...
export function WorkflowEditor({
  ...
  const prevAgentCountRef = useRef<number>(
    state.present.workflow.agents.length,
  );
  useEffect(() => {
  ...
  const [activePanel, setActivePanel] = useState<"playground" | "copilot">(
    "copilot",
  );
  ...
  const handleModeTransition = useCallback(
    (
      newMode: "draft" | "live",
      reason: "publish" | "view_live" | "switch_draft" | "modal_switch",
    ) => {
  ...
  const copilotRef = useRef<{ handleUserMessage: (message: string) => void }>(
    null,
  );
  ...
  const {
    ...
    onOpen: onRevertModalOpen,
    onClose: onRevertModalClose,
  } = useDisclosure();
  ...
  const [hasAgentInstructionChanges, setHasAgentInstructionChanges] =
    useState<boolean>(() => {
      if (typeof window === "undefined") return false;
      return (
        localStorage.getItem(`agent_instructions_changed_${projectId}`) ===
        "true"
      );
    });
  ...
  const markPlaygroundTested = useCallback(() => {
    if (!hasPlaygroundTested && hasAgentInstructionChanges) {
      // Only mark if step 1 is complete
  ...
  const triggerCopilotChat = useCallback(
    (message: string) => {
      setActivePanel("copilot");
      updateViewMode(
        viewMode === "three_all"
          ? "three_all"
          : viewMode === "two_agents_chat"
            ? "two_agents_skipper"
            : "two_chat_skipper",
      );
  ...
  function handleDeletePipeline(name: string) {
    if (
      window.confirm(`Are you sure you want to delete the pipeline "${name}"?`)
    ) {
  ...
  function handleAddAgentToPipeline(pipelineName: string) {
    ...
    const pipeline = state.present.workflow.pipelines?.find(
  ...
  async function handleDeleteAgent(name: string) {
    if (
      window.confirm(`Are you sure you want to delete the agent "${name}"?`)
    ) {
      ...
      if (!isLive) {
        try {
          const remainingAgents = state.present.workflow.agents.filter(
            (a) => a.name !== name,
          );
          const toSave = {
            ...
            startAgent:
              state.present.workflow.startAgent === name
  ...
  function handleAddPromptFromModal(
    prompt: Partial<z.infer<typeof WorkflowPrompt>>,
  ) {
    dispatchGuarded({ type: "add_prompt", prompt, fromCopilot: true });
  }

  function handleUpdatePromptFromModal(
    name: string,
    prompt: Partial<z.infer<typeof WorkflowPrompt>>,
  ) {
  ...
  async function handleDeletePrompt(name: string) {
    if (
      window.confirm(`Are you sure you want to delete the prompt "${name}"?`)
    ) {
      // Optimistically update UI (guard will show modal in live mode)
      ...
      if (!isLive) {
        try {
          const toSave = {
            ...state.present.workflow,
            prompts: state.present.workflow.prompts.filter(
              (p) => p.name !== name,
            ),
          } as z.infer<typeof Workflow>;
          await saveWorkflow(projectId, toSave);
  ...
  function handleReorderAgents(agents: z.infer<typeof WorkflowAgent>[]) {
    ...
    localStorage.setItem(
      `${mode}_workflow_${projectId}_agent_order`,
  ...
  function handleReorderPipelines(
    pipelines: z.infer<typeof WorkflowPipeline>[],
  ) {
    // Save order to localStorage
    const orderMap = pipelines.reduce(
      (acc, pipeline, index) => {
    ...
    const mode = isLive ? "live" : "draft";
    localStorage.setItem(
      `${mode}_workflow_${projectId}_pipeline_order`,
      JSON.stringify(orderMap),
    );
  ...
  useEffect(() => {
    if (
      pendingProjectName &&
      (projectConfig.name || "").trim().toLowerCase() ===
        pendingProjectName.trim().toLowerCase()
    ) {
  ...
  const WORKFLOW_MOD_ACTIONS = useRef(
    new Set([
      "add_agent",
      "add_tool",
      "add_prompt",
      "add_pipeline",
      "show_add_datasource_modal",
      "show_add_variable_modal",
      "show_add_agent_modal",
      "show_add_tool_modal",
      "update_agent",
      "update_tool",
      "update_prompt",
      "update_prompt_no_select",
      "update_pipeline",
      "delete_agent",
      "delete_tool",
      "delete_prompt",
  ...
  const dispatchGuarded = useCallback(
    (action: Action) => {
      ...
      if (
        WORKFLOW_MOD_ACTIONS.has((action as any).type) &&
        isLive &&
  ...
  useEffect(() => {
    ...
    if (
      !isLive &&
      lastWorkflowId &&
      lastWorkflowId.startsWith("live-") &&
  ...
  function handleTogglePanel() {
    if (
      ...
      if (viewMode === "two_agents_chat") updateViewMode("two_agents_skipper");
        ...
        updateViewMode("two_agents_chat");
      else if (viewMode === "two_chat_skipper")
        updateViewMode("two_chat_skipper");
    }
  }
  ...
  const handleProjectNameCommit = async (value: string) => {
    ...
    try {
      ...
      const isDuplicate = projects.some(
        (p) =>
          ((p as any).id ?? (p as any)._id) !== projectId &&
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col gap-5">
          ...
          <ModalContent>
            ...
            <ModalBody>
              <p className="text-gray-600 dark:text-gray-400">
                Seems like you&apos;re trying to edit the live workflow. Only
                the draft version can be modified. Changes will not be saved.
              </p>
            </ModalBody>
            <ModalFooter>
              <Button
        ...
        {!isHydrated ? (
          <ResizablePanelGroup
            key={`hydration-${viewMode}`}
            direction="horizontal"
            className="flex-1 flex overflow-auto gap-1 rounded-none bg-zinc-50 dark:bg-zinc-900"
          >
            {viewMode !== "two_chat_skipper" && (
              <ResizablePanel
            ...
            {viewMode !== "two_chat_skipper" && (
              <ResizableHandle
                withHandle
                className={`w-[3px] bg-transparent ${isLeftPanelCollapsed ? "hidden" : ""}`}
              />
            )}
            {(viewMode === "two_agents_chat" || viewMode === "three_all") && (
              <ResizablePanel
                minSize={20}
                defaultSize={getPanelRatios(viewMode).chatApp}
                id="chat"
                order={2}
            ...
            {(viewMode === "two_agents_skipper" ||
              viewMode === "three_all") && (
              <ResizablePanel
                minSize={20}
                defaultSize={getPanelRatios(viewMode).copilot}
                id="copilot"
                order={3}
                className="overflow-hidden"
              >
            ...
            {viewMode === "two_chat_skipper" && (
              <>
                ...
                <ResizableHandle
                  withHandle
                  className="w-[3px] bg-transparent"
                />
                <ResizablePanel
                  minSize={20}
                  defaultSize={getPanelRatios(viewMode).copilot}
          ...
          >
            ...
            {viewMode !== "two_chat_skipper" && (
              <ResizableHandle
                withHandle
                className={`w-[3px] bg-transparent ${isLeftPanelCollapsed && !state.present.selection ? "hidden" : ""}`}
              />
            )}
            ...
            <ResizablePanel
              ...
              id="chat"
              order={2}
              className={`overflow-hidden relative ${viewMode === "two_agents_skipper" ? "hidden" : ""}`}
            >
              <ChatApp
                  ...
                  <div className="h-full overflow-auto">
                    {state.present.selection?.type === "agent" && (
                      <AgentConfig
                        ...
                        workflow={state.present.workflow}
                        agent={
                          state.present.workflow.agents.find(
                            (agent) =>
                              agent.name === state.present.selection!.name,
                          )!
                        }
                        usedAgentNames={
                          new Set(
                            state.present.workflow.agents
                              .filter(
                                (agent) =>
                                  agent.name !== state.present.selection!.name,
                              )
                        ...
                        usedPipelineNames={
                          new Set(
                            (state.present.workflow.pipelines || []).map(
                      ...
                      (() => {
                        ...
                        return (
                          <ToolConfig
                            ...
                            usedToolNames={
                              new Set([
                                ...state.present.workflow.tools
                                  .filter(
                                    (tool) =>
                                      tool.name !==
                                      state.present.selection!.name,
                                  )
                                  .map((tool) => tool.name),
                              ])
                            }
                    ...
                    {state.present.selection?.type === "prompt" && (
                      <PromptConfig
                        key={`overlay-${state.present.selection.name}-${configKey}`}
                        prompt={
                          state.present.workflow.prompts.find(
                            (prompt) =>
                              prompt.name === state.present.selection!.name,
                          )!
                        }
                        agents={state.present.workflow.agents}
                        tools={(() => {
                        ...
                        usedPromptNames={
                          new Set(
                            state.present.workflow.prompts
                              .filter(
                                (prompt) =>
                                  prompt.name !== state.present.selection!.name,
                              )
                    ...
                    {state.present.selection?.type === "datasource" && (
                      <DataSourceConfig
                        ...
                        handleClose={() =>
                          dispatch({ type: "unselect_datasource" })
                        }
                    ...
                    {state.present.selection?.type === "pipeline" && (
                      <PipelineConfig
                        ...
                        workflow={state.present.workflow}
                        pipeline={
                          state.present.workflow.pipelines?.find(
                            (pipeline) =>
                        ...
                        usedPipelineNames={
                          new Set(
                            (state.present.workflow.pipelines || [])
                              .filter(
                                (pipeline) =>
                                  pipeline.name !==
                                  state.present.selection!.name,
                        ...
                        usedAgentNames={
                          new Set(
                            state.present.workflow.agents.map(
                              (agent) => agent.name,
                            ),
                          )
                        }
                        agents={state.present.workflow.agents}
                        pipelines={state.present.workflow.pipelines || []}
                        handleUpdate={handleUpdatePipeline.bind(
                          null,
                          state.present.selection.name,
                        )}
                        handleClose={() =>
            ...
            <ResizablePanel
              ...
              defaultSize={getPanelRatios(viewMode).copilot}
              id="copilot"
              order={viewMode === "three_all" ? 3 : 2}
              className={`overflow-hidden relative ${viewMode === "two_agents_chat" ? "hidden" : ""}`}
            >
              <Copilot
                ref={copilotRef}
                projectId={projectId}
            ...
            stepsOverride={[
              {
                ...
                title: "Step 1/5",
                content:
                  "Use Copilot to create and refine agents. Describe what you need, then iterate with its suggestions.",
              },
              {
                target: "entity-agents",
                title: "Step 2/5",
                content:
                  "All your agents appear here. Adjust instructions, switch models, and fine-tune their behavior.",
              },
              {
                target: "entity-tools",
                title: "Step 3/5",
                content:
                  "Pick from thousands of ready-made tools or connect your own MCP servers.",
              },
              {
                target: "entity-data",
                title: "Step 4/5",
                content:
                  "Upload files, scrape websites, or add free-text knowledge to guide your agents.",
              },
              {
                target: "entity-prompts",
                title: "Step 5/5",
                content:
                  "Define reusable context variables automatically shared across all agents.",
              },
            ]}
            onStepChange={(_, step) => {
            ...
            stepsOverride={[
              {
                ...
                title: "Step 1/2",
                content:
                  "Chat with your assistant to test it. Send messages, watch tool calls in action, and debug agent flows.",
              },
              {
                target: "copilot",
                title: "Step 2/2",
                content:
                  'Ask Copilot to improve your agents based on test results. Use "Fix" and "Explain" to iterate quickly.',
              },
            ]}
            onStepChange={(index) => {
              if (index === 0) {
            ...
            stepsOverride={[
              {
                target: "playground",
                title: "Step 1/5",
                content: "Chat: you can chat with your assistant here.",
              },
              {
                target: "triggers",
                title: "Step 2/5",
                content:
                  "Triggers: set up external (webhook/integration) or time-based schedules.",
              },
              {
                target: "jobs",
                title: "Step 3/5",
                content:
                  "Jobs: monitor your trigger runs and scheduled tasks here.",
              },
              {
                target: "settings",
                title: "Step 4/5",
                content:
                  "Settings: find API keys to connect with the API and SDK.",
              },
              {
                target: "conversations",
                title: "Step 5/5",
                content:
                  "Conversations: see all past interactions in one place, including manual chats, trigger activity, and API calls.",
              },
            ]}
            onStepChange={(index) => {
              if (index === 0) {
        ...
        <Modal isOpen={isRevertModalOpen} onClose={onRevertModalClose}>
          <ModalContent>
            ...
            <ModalBody>
              <p>
                Are you sure you want to revert to the live workflow? This will
                discard all your current draft changes and switch back to the
                live version.
              </p>
            </ModalBody>
            <ModalFooter>
