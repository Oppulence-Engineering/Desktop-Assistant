...
import {
  ...
  USE_RAG_SCRAPING,
  USE_BILLING,
} from "@/app/lib/feature_flags";
import { notFound } from "next/navigation";
import { requireActiveBillingSubscription } from "@/app/lib/billing";
import { container } from "@/di/container";
import { getEligibleModelsCached } from "@/app/lib/billing";
...
const listScheduledJobRulesController = container.resolve<IListScheduledJobRulesController>(
  "listScheduledJobRulesController",
);
const listRecurringJobRulesController = container.resolve<IListRecurringJobRulesController>(
  "listRecurringJobRulesController",
);
const listIntegrationTriggerDeploymentsController =
  container.resolve<IListIntegrationTriggerDeploymentsController>(
    "listIntegrationTriggerDeploymentsController",
  );
...
export default async function Page(props: { params: Promise<{ projectId: string }> }) {
  ...
  if (USE_BILLING) {
    eligibleModels = await getEligibleModelsCached(customer.id);
  }
