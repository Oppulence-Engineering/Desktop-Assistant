...
import { IIntegrationTriggerDeploymentsRepository } from "@/src/application/repositories/integration-trigger-deployments.repository.interface";
...
const WEBHOOK_SECRET = process.env.INTEGRATION_TRIGGERS_WEBHOOK_SECRET || "test";
...
export interface IHandleIntegrationWebhookRequestUseCase {
    execute(request: z.infer<typeof requestSchema>): Promise<void>;
}

export class HandleIntegrationWebhookRequestUseCase implements IHandleIntegrationWebhookRequestUseCase {
    ...
    async execute(request: z.infer<typeof requestSchema>): Promise<void> {
        ...
        const logger = new PrefixLogger(`integration-trigger-webhook-[${event.type}]-[${event.data.trigger_nano_id}]`);

        // fetch trigger deployment data from db
        const deployment = await this.integrationTriggerDeploymentsRepository.fetchByIntegrationTriggerId(event.data.trigger_nano_id);
        if (!deployment) {
        ...
        if (USE_BILLING) {
            ...
            // log usage for integration trigger
            await logUsage(billingCustomerId, {
                items: [{
                    type: "INTEGRATION_TRIGGER_USAGE",
                    triggerSlug: deployment.triggerTypeSlug,
                    context: "trigger.integration",
                }],
            });
        }
