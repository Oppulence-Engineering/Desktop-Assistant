...
const TOUR_STEPS: TourStep[] = [
  {
    target: "copilot",
    content:
      "Build agents with the help of copilot.\nThis might take a minute.",
    title: "Step 1/9",
  },
  {
    target: "playground",
    content:
      "Test your assistant in the playground.\nDebug tool calls and responses.",
    title: "Step 2/9",
  },
  {
...
function TourBackdrop({ targetElement }: { targetElement: Element | null }) {
  ...
  const isPanelTarget =
    ...
    [
      ...
      "jobs",
      "conversations",
    ].includes(targetElement.getAttribute("data-tour-target")!);
  ...
  return (
    <>
      {/* Top */}
      <div
        className="fixed z-100 backdrop-blur-sm bg-black/30"
        style={{
          top: 0,
          left: 0,
          right: 0,
      ...
      <div
        className="fixed z-100 border-2 border-white/50 rounded-none pointer-events-none"
        style={{
...
export function ProductTour({
  ...
  const steps =
    stepsOverride && stepsOverride.length > 0 ? stepsOverride : TOUR_STEPS;
  ...
  const { x, y, strategy, refs, context, middlewareData } = useFloating({
    ...
    middleware: [
      offset(16),
      flip({
        fallbackPlacements: isPanelTarget
          ? ["left", "top", "bottom"]
          : ["bottom", "left", "right"],
        padding: 16,
      }),
    ...
    raf1 = requestAnimationFrame(() => {
      raf2 = requestAnimationFrame(() => {
        const el = document.querySelector(
          `[data-tour-target="${currentTarget}"]`,
        );
        setTargetElement(el);
  ...
  return (
    <FloatingPortal>
      ...
      <div
        ...
        className="bg-white dark:bg-zinc-800 rounded-none shadow-lg border border-zinc-200 dark:border-zinc-700 p-4 animate-in fade-in duration-200"
      >
        <button
          onClick={handleSkip}
        ...
        <div
          className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-3 whitespace-pre-line [&>a]:underline"
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={handleNext}
            className="px-4 py-1.5 bg-indigo-600 text-white rounded-none text-sm font-medium hover:bg-indigo-700"
          >
