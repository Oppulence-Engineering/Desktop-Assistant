...
export function ComposeBoxCopilot({
  ...
  return (
    <div className="relative group z-10">
      ...
      <div
        className="absolute -top-6 right-0 text-xs text-gray-500 dark:text-gray-400 opacity-0
      ...
      <div
        className="rounded-none border-[1.5px] border-gray-200 dark:border-[#2a2d31] relative
...
function CopilotStatusBar({
  ...
  const renderContext = () => {
    ...
    if (context.type === "chat")
      icon = (
        <svg
          className="w-3.5 h-3.5 text-blue-500 dark:text-blue-300 mr-1"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M18 10c0 3.866-3.582 7-8 7a8.96 8.96 0 01-4.39-1.11L2 17l1.11-2.61A8.96 8.96 0 012 10c0-3.866 3.582-7 8-7s8 3.134 8 7z" />
        </svg>
      );
    if (context.type === "agent")
      icon = (
        <svg
          className="w-3.5 h-3.5 text-green-500 dark:text-green-300 mr-1"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M10 2a6 6 0 016 6c0 2.21-1.343 4.09-3.25 5.25A4.992 4.992 0 0110 18a4.992 4.992 0 01-2.75-4.75C5.343 12.09 4 10.21 4 8a6 6 0 016-6z" />
        </svg>
      );
    if (context.type === "tool")
      icon = (
        <svg
          className="w-3.5 h-3.5 text-yellow-500 dark:text-yellow-300 mr-1"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
          <path d="M13.293 2.293a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-8.5 8.5a1 1 0 01-.293.207l-4 2a1 1 0 01-1.316-1.316l2-4a1 1 0 01.207-.293l8.5-8.5z" />
        </svg>
      );
    if (context.type === "prompt")
      icon = (
        <svg
          className="w-3.5 h-3.5 text-purple-500 dark:text-purple-300 mr-1"
          fill="currentColor"
          viewBox="0 0 20 20"
        >
    ...
    return (
      <div className="flex items-center gap-1 px-2 py-1 rounded-none border border-zinc-200 dark:border-zinc-700 bg-zinc-50/70 dark:bg-zinc-800/40 shadow-sm text-xs font-medium text-zinc-700 dark:text-zinc-200 max-w-[180px] truncate">
        {icon}
    ...
    return (
      <div className="flex flex-col min-w-0">
        ...
        <span className="text-xs text-gray-500 dark:text-gray-400">
          {appliedCount ?? 0} applied, {pendingCount ?? 0} pending
        </span>
      </div>
    );
  };
  ...
  const renderApplyAll = () => {
    ...
    if (!allCardsLoaded) {
      return (
        <Tooltip
          content="Apply all will be available when all changes are ready"
          placement="top"
        >
  ...
  return (
    <div className="w-auto max-w-[calc(100%-16px)] mx-auto flex items-center px-3 py-1 pt-2.5 pb-5 mt-2 -mb-3 rounded-none bg-white/90 dark:bg-zinc-800/90 border border-zinc-300 dark:border-zinc-700 shadow-md dark:shadow-zinc-950/10 backdrop-blur-sm transition-all z-0 relative mx-2 overflow-visible">
