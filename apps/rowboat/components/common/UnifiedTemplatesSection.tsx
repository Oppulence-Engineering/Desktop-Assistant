...
export function UnifiedTemplatesSection({
  ...
  const [selectedType, setSelectedType] = useState<"prebuilt" | "community">(
    SHOW_COMMUNITY_PUBLISH ? "prebuilt" : "prebuilt",
  );
  const [selectedCategories, setSelectedCategories] = useState<Set<string>>(
    new Set(),
  );
  const [sortBy, setSortBy] = useState<"popular" | "newest" | "alphabetical">(
    "alphabetical",
  );
  ...
  const [hasUserInteractedWithLikes, setHasUserInteractedWithLikes] =
    useState(false);
  const [originalOrder, setOriginalOrder] = useState<Map<string, number>>(
    new Map(),
  );
  ...
  const hasMore =
    ...
    (!!onLoadMore && filteredTemplates.length >= visibleCount);
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-none p-6 shadow-lg border border-gray-200 dark:border-gray-700">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-none p-6 shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="text-center py-12">
          ...
          {onRetry && (
            <button
              onClick={onRetry}
              className="mt-4 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-none hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
            >
              Try Again
            </button>
          )}
    ...
    <div className="bg-white dark:bg-gray-800 rounded-none p-6 shadow-lg border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex-1">
            <Input
              ...
              classNames={{
                ...
                inputWrapper:
                  "focus-within:ring-0 focus-within:ring-offset-0 focus-within:border-gray-300 dark:focus-within:border-gray-600",
              }}
            />
          </div>
          ...
          <div className="flex gap-2">
            ...
            <div className="relative">
              ...
              <div className="pointer-events-none absolute inset-y-0 right-3 flex items-center">
                <svg
                  className="w-4 h-4 text-gray-400 -translate-y-[2px]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
        ...
        {filteredTemplates.length === 0 ? (
          ...
          <>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Showing {Math.min(visibleCount, filteredTemplates.length)} of{" "}
              {filteredTemplates.length} template
              {filteredTemplates.length !== 1 ? "s" : ""} ({rowsShown} row
              {rowsShown !== 1 ? "s" : ""})
            </div>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-900 rounded-none shadow-xl border border-gray-200 dark:border-gray-700 max-w-sm w-full p-5">
            <div className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">
              Delete template?
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
              This will permanently remove &quot;{pendingDeleteItem?.name}&quot;
              from the community templates. This action cannot be undone.
            </div>
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                className="px-4 py-2 text-sm rounded-none bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 text-sm rounded-none bg-red-600 hover:bg-red-700 text-white"
              >
                Delete
              </button>
            </div>
