# [Complete] RFC 027: Durable Agent Runtime

|                  |                                                                                                                                                                                                                                                                                                                                                       |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**          | 027                                                                                                                                                                                                                                                                                                                                                   |
| **Status**       | Implemented & enabled by default (active wherever Temporal is on; no master flag) — see [Implementation status](#implementation-status)                                                                                                                                                                                                               |
| **Track**        | Backend service plane · durable agentic runtime                                                                                                                                                                                                                                                                                                       |
| **Owners**       | `apps/rowboat-api`                                                                                                                                                                                                                                                                                                                                    |
| **Created**      | 2026-06-17                                                                                                                                                                                                                                                                                                                                            |
| **Last updated** | 2026-06-17                                                                                                                                                                                                                                                                                                                                            |
| **Depends on**   | [RFC 004](./complete-004-cloud-agent-runtime.md) cloud runtime · [RFC 010](./complete-010-rowboat-api-service-plane.md) service plane · [RFC 012](./012-connector-suite-and-consent-broker.md) consent broker · [RFC 005](./complete-005-temporal-schedule-integration.md) Temporal Schedules · ent/Postgres · Temporal · AI Gateway (`internal/llm`) |
| **Related**      | [RFC 008](./008-conduit-eigen-faculties.md) faculties · [RFC 018](./018-a2a-delegation-and-agent-identity.md) A2A delegation · [RFC 021](./complete-021-semantic-memory-index.md) semantic memory · [RFC 023](./023-closed-loop-actions.md) closed-loop actions · [RFC 025](./025-desktop-runtime-durability.md) desktop runtime durability                    |
...
| `skills/*` (load-on-demand)    | A retrievable knowledge tool (`skill.load`) or sub-prompt fetched by an activity; v1 keeps it minimal             | tool mechanics; [RFC 021](./complete-021-semantic-memory-index.md) for recall                           |
...
| `connections/*` (MCP/OpenAPI)  | `OAuthConnection`/`MCPConnection`/`legacy integration vendorLink` + consent broker; creds resolved **inside** the tool activity    | `internal/connectors`, `crypto.Sealer`, [RFC 012](./012-connector-suite-and-consent-broker.md) |
...
Mounted in `cmd/server/wire.go` `mountRoutes` inside the authenticated group (after `authMW.RequireJWT` at `wire.go:335`, alongside `/v1/background-tasks` at `:349`), wherever Temporal is wired (`temporalClient != nil`), with a new `ratelimit.GroupAgent`:
...
> **Shipped state:** the phases below were the rollout plan. On completion the temporary master flag was **removed** — the durable agent runtime is now active **wherever Temporal is enabled** (the same gating as the cloud runtime / event router; it cannot run without durable workflows), and the capability sub-flags `AGENT_STREAMING_ENABLED` / `AGENT_HITL_ENABLED` / `AGENT_SUBAGENTS_ENABLED` **default on**. There is no separate opt-in: turning on the durable backend turns on the agent runtime.

The original plan used a master flag `AGENT_RUNTIME_ENABLED=false` (the new-feature default, like `TEMPORAL_ENABLED`/`CLOUD_SCHEDULER_ENABLED`/`CLOUD_EVENTS_ROUTING_ENABLED` at `config.go:390-412`) + the three sub-flags. Each phase ended in a hard gate.
...
| **P5** | Channel adapters (Slack/etc.) start/append sessions via `internal/agentsessions` (reuse cloudevents webhook+router pattern); Temporal Schedules start sessions (reuse RFC 005); `AgentDefinition` CRUD GA; **remove the master flag** so the runtime is on wherever Temporal is. `sandbox/*` explicitly deferred. | Scheduled + channel-triggered sessions run desktop-closed; SLOs hold over soak; rollback = disable Temporal or the capability sub-flags.                      |
...
Implemented in `apps/rowboat-api`, **enabled by default**: there is no master flag — the runtime is active wherever Temporal is enabled (`temporalClient != nil` on the server; always on the worker, which requires Temporal). The capability sub-flags (`AGENT_STREAMING_ENABLED`/`AGENT_HITL_ENABLED`/`AGENT_SUBAGENTS_ENABLED`) default on; deny-by-default at the tool/agent level still governs what any individual agent can actually do. The reason→act loop runs in workflow code; each LLM/tool call is a checkpointed activity.

| Area                                                                                                                                                                                                                  | Status | Where                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P0** — ent entities + additive migration; config flags; embedded built-in loader; worker registration (inert until flagged)                                                                                         | ✅     | `ent/schema/agent_*.go`, `migrations/0001_init.sql`, `internal/appconfig/config.go` (`Agent*`), `internal/agentregistry/builtins/agents/*`, `internal/db/interceptors.go` (tenant scoping for all six entities)                                                                                                                                                                                                                                                                                             |
| **P1** — durable loop in workflow code; `LLMComplete`/`ToolInvoke`/`AppendSessionEvent` activities; deny-by-default; per-turn limits; `POST /v1/agent-sessions`, `POST …/turns`, `GET …/events`                       | ✅     | `internal/agentworkflow/{workflow,activities,types}.go`, `internal/agentsessions/starter.go`, `internal/agents/handler.go`                                                                                                                                                                                                                                                                                                                                                                                  |
| **P2** — long-lived idle-between-turns workflow; `submitTurn` **Update** (synchronous ack + turn seq + continuation token); ContinueAsNew (compacted state, stable workflow id); NDJSON SSE + Redis bus + DB backfill | ✅     | `internal/agentworkflow/workflow.go` (`AwaitWithTimeout`, `NewContinueAsNewError`), `internal/agentstream/{bus,streamer}.go`, `internal/agentworkflow/starter.go`                                                                                                                                                                                                                                                                                                                                           |
| **P3** — `ToolDef` trust tiers (RFC 012); approval-required tools pause on `workflow.Await` (zero worker slot); `approveAction` Update + `ValidateApproval`; `AgentApproval` rows + events                            | ✅     | `internal/backgroundtaskruntime/runtime.go` (`ToolDef.TrustTier`), `internal/agentworkflow/workflow.go` (`awaitApproval`)                                                                                                                                                                                                                                                                                                                                                                                   |
| **P4** — `rowboat.agent.subagent.v1` child workflow; `subagent.delegate` → `ExecuteChildWorkflow` with isolated context + narrowed registry; `ParentClosePolicy=REQUEST_CANCEL`; depth/fan-out caps                   | ✅     | `internal/agentworkflow/workflow.go` (`SubagentWorkflow`, `handleSubagent`, `ResolveSubagent`)                                                                                                                                                                                                                                                                                                                                                                                                              |
| **P5** — `AgentDefinition` CRUD; channel adapters + Temporal Schedules via the canonical starter                                                                                                                      | ✅     | CRUD: `internal/agents/definitions.go`. Channels: `internal/agentchannels` (generic internal ingest + a Slack adapter reusing `cloudevents.VerifySlackSignature`), threading a conversation to one session via `agentsessions.Starter.CreateOrContinue` keyed by `(channel, channel_key)`. Schedules: `agentworkflow.ScheduledSessionWorkflow` + `SessionScheduler` (Temporal Schedule action mirroring RFC 005) → `agentsessions.Starter.StartScheduledSession`; `POST/DELETE /v1/agents/{slug}/schedule`. |

**RFC 012 / signing (now implemented):** money-moving approvals require an HMAC-signed, claim-bound, MFA-asserting per-invocation token (`internal/agenttoken`), minted at `POST …/approvals/{approvalId}/token` (MFA read from the verified WorkOS `amr`/`acr` claim) and verified in `ActivityValidateApproval`. Continuation tokens are HMAC-signed (owner-bound) and accepted as an alternative session resolver. Oversized tool results are claim-checked to `AgentToolResultBlob` with a reference + preview in the transcript and a read-tier `tool_result.read` capability to page through them.

**Tests** (all green, `internal/agent*/*_test.go`): replay/determinism + Register-by-name mirror, single- + multi-turn (`submitTurn` Update), reason→act tool loop, deny-by-default (`shell` denied), billing-idempotency anchor, HITL approve + invalid-token reject + signed-token binding/MFA, subagent delegation, ContinueAsNew cadence + stable id, at-least-once event idempotency (unique `(session, seq)`), SSE `?afterSeq` backfill, continuation-token resolve/forge-reject, claim-check spill + read, channel thread→one-session + binding resolution, scheduled-session fire, tenant isolation, migration apply (sqlite), and the **sandbox Non-Goal guard** (no shell/exec/eval capability may be registered).

**Deferred (genuinely out of scope):**

- **Sandbox / code-exec** — explicit Non-Goal, enforced by a guard test. Untrusted/model-generated code execution needs an isolated compute boundary (gVisor/Firecracker/microVM) before it can be enabled; deny-by-default refuses any `shell`/`exec`/`eval` tool until then.
- **WorkOS MFA provider depth** — the step-up is read from the verified token's `amr`/`acr` claim and gated by `AGENT_REQUIRE_MFA_FOR_MONEY_MOVING`; a dedicated WorkOS step-up _challenge_ API call is a thin future addition behind the same seam.
- **Object-store claim-check** — large results spill to the `AgentToolResultBlob` ent table today; moving the bytes to an object store (keeping the reference contract) is a capacity follow-up.
