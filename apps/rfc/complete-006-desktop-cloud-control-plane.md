# [Complete] RFC 006: Desktop as Cloud Workflow Control Plane

|                  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**          | 006                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Status**       | Complete — implemented on `feat/rfc-006-desktop-control-plane` + `feat/rfc-006-deferred-items`: `GET /v1/background-tasks/{slug}/schedule-state` (composes triggers + RFC 002 schedule-state rows + RFC 005 sync state/Describe into per-source mechanism/health/next-due), `bg-task:getCloudScheduleState` + `CloudScheduleStatus` chip (60s cadence), explicit `Cloud scheduled` vs `Runs when desktop is open` labels + execution-target consequence labels, offline-return (seen-marker sidecar, boot sweep, bounded sidecar-gated artifact auto-pull, `bg-task:offlineRuns` toast), and task/time-range filters on the global runs view. Implementation note vs this draft: the desktop scheduler now SKIPS api-target timed triggers entirely (the RFC 001 loop / RFC 005 Schedules own them; desktop-open firing double-ran every occurrence). The follow-up branch closed all three deferred items: the single-run GET attaches `sourceEvent` (opt-in `WithCloudEvent` preload; lists/status stay slim) and the transcript renders "Triggered by: \<event\> — \<subject\>" via a one-shot `bg-task:getCloudRun`; an opt-in system notification for missed cloud runs (Settings → Notifications, default OFF, `notifications.json`); and a one-time-suppressible quit reminder for pending desktop schedules sharing ONE `before-quit` handler with the cleanup (preventDefault doesn't stop sibling listeners), guarded against auto-update restarts and early startup quits. Nothing remains deferred. |
| **Track**        | Cloud-native background workflows                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Owners**       | `apps/x` (Electron: main + renderer + core)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Created**      | 2026-06-05                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Last updated** | 2026-06-11                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Depends on**   | [RFC 001](./complete-001-api-owned-scheduler.md) (schedule state to display), [RFC 002](./complete-002-durable-schedule-state.md) (`next due`/`last evaluated`), [RFC 005](./complete-005-temporal-schedule-integration.md) (schedule health), [RFC 003](./complete-003-cloud-event-ingestion.md) (event→run linkage)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Supersedes**   | Former cloud workflow planning desktop-control-plane sections.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

## Summary

As scheduling and execution move into the API ([RFC 001](./complete-001-api-owned-scheduler.md),
[003](./complete-003-cloud-event-ingestion.md), [005](./complete-005-temporal-schedule-integration.md)), the
...
- A full admin console (the [RFC 003](./complete-003-cloud-event-ingestion.md) event browser, deep
  Temporal internals).
...
sequenceDiagram
    ...
    participant API as Oppulence API
...
- **Event→run link → shown in the run transcript** ("Triggered by: email from Acme") once the
  [RFC 003](./complete-003-cloud-event-ingestion.md) `cloud_event_id` linkage ships. High value, low
