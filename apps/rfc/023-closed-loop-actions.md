...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**               | 023                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Status**            | Implemented (backend) — the full propose→approve→execute→watch loop ships in `apps/rowboat-api`. **WP1:** the propose-only `action.propose` runtime tool (`internal/backgroundtaskruntime/tools_action.go`; the model proposes, never executes). **WP2:** the single-use, params-bound, expiring approval token (`internal/actions/token.go`) — "no money moves without a token", enforced by verify+atomic-consume. **WP3:** the HTTP product Act-seam executor (`internal/actions/executor_http.go`), wired when `ACTION_ACT_SEAM_BASE_URL` is set (nil ⇒ fail closed). **WP4:** the CloudEvent Watch leg — `cloud_event.correlation_id` + product sources, `Broker.CorrelateReturn`, and the router re-triggering the originating live-note (`internal/cloudevents/router.go`). Surface mounts at `/v1/action-proposals` and `/v1/objects/{ref}/audit` behind `ACTIONS_ENABLED` (default off). An end-to-end round-trip test proves the loop with a full, queryable audit chain. **Operator UI:** the **Actions** console in the rowboat-www dashboard (`components/actions/*`) — the pending-proposal queue with approve→token→execute and the per-object proposal→token→execution→return-event audit trail. RFC 023 is complete end to end; still ships dark behind `ACTIONS_ENABLED` until a product Act seam is connected. |
