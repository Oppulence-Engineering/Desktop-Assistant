...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**               | 023                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Status**            | Partially implemented — the propose→approve→execute broker and the single-use, params-bound, expiring approval token (the "no money moves without a token" guarantee) ship in `apps/rowboat-api/internal/actions` (+ `internal/actionmetrics`), mounted at `/v1/action-proposals` and `/v1/objects/{ref}/audit` behind `ACTIONS_ENABLED` (default off). The `cloud_event` source enum is extended with the product Act-seam sources. **Deferred (WP-remainder):** the product Act-seam executor (`Executor` is nil until a product MCP is wired — execute fails closed), the WP1 runtime `propose-action` tool, the WP4 CloudEvent Watch correlation + live-note re-trigger, and the desktop approval-card UI. |
