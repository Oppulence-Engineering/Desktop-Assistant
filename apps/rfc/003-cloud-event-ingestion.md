...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**          | 003                                                                                                                                                 |
| **Status**       | Implemented (stages 1–5) on `rfc-003-cloud-event-ingestion` — ingestion + dedupe, Temporal route workflow + two-pass router, fixtures, Google + Slack webhooks, the self-serve Slack workspace connect flow (`/oauth/slack/start` → `/v1/slack-oauth/claim`), and the Google watch manager (`internal/googlewatch`, scheduler process) that registers/renews the Gmail `users.watch` + Calendar channels so pushes actually flow. Staging soak (stage 6) pending. |
