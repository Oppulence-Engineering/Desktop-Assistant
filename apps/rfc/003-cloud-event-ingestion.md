...
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**          | 003                                                                                                                                                 |
| **Status**       | Implemented (stages 1–5) on `rfc-003-cloud-event-ingestion` — ingestion + dedupe, Temporal route workflow + two-pass router, fixtures, Google + Slack webhooks, and the self-serve Slack workspace connect flow (`/oauth/slack/start` → `/v1/slack-oauth/claim`). Staging soak (stage 6) pending. |
