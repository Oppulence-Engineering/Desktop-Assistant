# [Complete] RFC 004: Cloud-Safe Agent Runtime for API Background Tasks

|                  |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**          | 004                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Status**       | Complete — implemented on `feat/rfc-004-cloud-runtime`: `internal/backgroundtaskruntime` (bounded loop, deny-by-default tools incl. Gmail/Calendar reads, staged artifact, classified non-retryable failures), `llm.ChatComplete` tool calling, `internal/googleapi` extraction, worker wiring. Implementation notes vs this draft: `EventSink` gained a third `ProgressEvent` method (needed for byte parity of the progress-90 + `temporal.artifact_updated` pairing); `CLOUD_RUNTIME_ENABLED` ships **enabled by default** (decision override of the ship-dark stance below — `false` remains the instant NoopRuntime rollback); a `CLOUD_RUNTIME_MODEL` default-model knob was added. Post-review hardening (`fix/rfc-004-review-findings`): LLM request ids are seeded by the Temporal activity **attempt**, so retries reserve fresh quota instead of failing terminally on replay (re-billing a retried attempt is the accepted cost); a scheduler-hosted reaper refunds reservations orphaned by a process death; tool results truncate into explicit JSON envelopes (`artifact.read` gets a dedicated 256 KiB budget) and an empty final message never overwrites the prior artifact; deadline breaches mid-tool classify as `runtime_deadline_exceeded`; worker vendor deps are built once with transient Infisical failures retried. Gap closure (2026-06-10): `notify.enqueue` descoped to Deferred (no notification service exists in the API); the full-workflow Temporal test-env integration test from the test plan landed in `internal/backgroundtaskworkflow/workflow_integration_test.go`. |
...
| **Consumed by**  | [RFC 001](./complete-001-api-owned-scheduler.md) & [RFC 003](./complete-003-cloud-event-ingestion.md) (their runs execute through this runtime), [RFC 007](./007-production-cloud-enablement.md) (must land before unbounded cloud tools GA)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- Run useful background jobs fully in the API worker (LLM-generated artifacts, connector
  reads).
- LLM access through the **Oppulence API gateway** (`internal/llm`) so billing/credits/quota
...
| `event.read`                       | linked `CloudEvent` payload ([RFC 003](./complete-003-cloud-event-ingestion.md)) | run-scoped, read-only              |
...
- **v1 connector read tools → Gmail + Google Calendar (read-only)**, matching the first
  [RFC 003](./complete-003-cloud-event-ingestion.md) event sources. Other connectors are additive
...
- `notify.enqueue` — descoped from the v1 allowlist (2026-06-10): no notification service
  exists in the API, so the tool was never implemented. Reintroduce as an additive
  deny-by-default registry entry once a notification path ships.
