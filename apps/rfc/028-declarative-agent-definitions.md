...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RFC**          | 028                                                                                                                                                                                                                                                                                                                                         |
| **Status**       | Implemented (server P0–P4; YAML authoring on by default) — see [Implementation status](#implementation-status)                                                                                                                                                                                                                              |
...
Implemented in `apps/rowboat-api` on top of RFC 027. YAML/JSON authoring is **on by default** wherever the agent runtime is (no master flag — it's just an additional Content-Type + the shared validator on `/v1/agents`). Two conditional pieces stay flagged: `AGENT_DECLARATIVE_TOOLS_ENABLED` (default off — the OpenAPI/MCP executor is RFC 020, not yet built) and `AGENT_GITOPS_ENABLED` (default off — opt-in ops mode that makes a repo authoritative).

| Phase                                                                                                                                                                                                                                                                                                                                                               | Status           | Where                                                                                                                                                                                                                                                                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P0** — `agentspec` types + strict YAML→JSON decoder + `Compile`/`ContentHash`; shared semantic validator; built-ins converted to YAML                                                                                                                                                                                                                             | ✅               | `internal/agentspec/{spec,compile}.go`, `internal/agentregistry/yaml.go` (`ValidateCompiled`, `LoadYAML`), `internal/agentregistry/builtins/agents/*/agent.yaml`                                                                                                                  |
| **P1** — ent fields (`source_format`/`raw_source`/`content_hash`/`managed_by`/`agent_sync_state`/`tools_json`, `AgentSession.agent_revision`, enthistory on `AgentDefinition`); `PUT /v1/agents/{slug}` (yaml/json), `POST /v1/agents/validate`, `GET ?format=yaml`, `/revisions`, `/rollback`; content-hash idempotency + concrete-revision bump; revision pinning | ✅               | `ent/schema/agent_definition.go`, `internal/agentregistry/persist.go`, `internal/agents/yaml.go`, `internal/agentsessions/starter.go`                                                                                                                                             |
| **P3** — declarative-tool spec (`kind: openapi\|mcp` + `manifestRef`), trust-gated (default-approval override applied at runtime) behind `AGENT_DECLARATIVE_TOOLS_ENABLED`; execution is the RFC 020 seam                                                                                                                                                           | ✅ (spec + gate) | `internal/agentregistry/yaml.go` validator, `internal/agentsessions/starter.go` (`applyApprovalOverrides`)                                                                                                                                                                        |
| **P4** — GitOps reconciler: `LoadDir` + `ReconcileOnce` (two-phase validate-all → persist-all, cross-agent cycle detection, drift repair, `agent_sync_state`), `managed_by=gitops` API-edit protection; internal `POST /v1/internal/agent-gitops/reconcile`                                                                                                         | ✅               | `internal/agentgitops/reconciler.go`, `internal/agentregistry/persist.go` (`ErrManagedByGitops`)                                                                                                                                                                                  |
| **P2** — published JSON Schema (one schema, three consumers); `rowboat agent validate\|diff\|push\|pull` CLI                                                                                                                                                                                                                                                        | ◑                | Schema: `apps/rowboat-api/api/agent.schema.json`. CLI: `apps/cli/src/agents/yaml-cli.ts` (+ `npm run agent`) — self-contained (node + `yaml` + fetch), **not compile-verified in this environment** (the CLI has no installed toolchain); run `npm i && npm run build` to verify. |

**Tests** (Go side, all green): decode/compile + strict-decode (unknown field, duplicate key) + content-hash stability (`agentspec`); shared-validator rejects (`tool_not_registered`, `model_not_allowed`, `limit_over_cap` not-clamped, `invalid_scope`, `subagent_cycle`/`not_found`, `declarative_tools_disabled`/`manifest_ref_required`) + built-in YAML round-trip (`agentregistry`); PUT apply/idempotent-no-op/revision-bump/slug-mismatch/unknown-tool, validate dry-run (persists nothing), `?format=yaml` round-trip, revisions + rollback, gitops 409 (`agents`); reconciler create/unchanged/**drift-repair**/removed→out_of_sync/cycle/invalid-tool (`agentgitops`); revision pinning (`agentsessions`); migrations apply on sqlite.

**Notable implementation detail:** revision bumps use a **concrete** `SetRevision(n+1)` rather than ent's atomic `AddRevision(1)`, because enthistory snapshots the field value present in the mutation — an atomic increment leaves the history row's `revision` label stale, breaking `/revisions` + rollback.

**Deferred (genuinely out of scope):** declarative-tool _execution_ (the generic OpenAPI/MCP activity) is RFC 020; the git-checkout _source_ (clone/poll) that feeds the reconciler is deployment wiring (a sidecar/CI posts the declared set to the internal endpoint); the TS CLI is provided but compile-verification needs the CLI's own toolchain.
