...
| `CloudEvent.source` enum **now includes** product sources                                     | `apps/rowboat-api/ent/schema/cloud_event.go` — enum carries `conduit, cadence, eigen, corinthian, canvas` alongside `gmail, google_calendar, google_drive, slack, webhook, mcp, github, linear, stripe, internal`. This row previously said products were missing; that is no longer true. |
...
- An **org-scoped shared spine** in `rowboat-api` so a team shares entity identities and cross-links. For general knowledge entities this remains a bounded projection. Customer relationships follow [RFC 036](./036-relationship-state-engine.md): observations, assertions, shared state, and encrypted evidence are backend-authoritative.
...
- Replacing the Markdown vault for user-authored knowledge. The vault remains canonical for local notes. Customer-relationship state is the explicit exception: the backend model from RFC 036 is canonical and notes link to it as evidence or working memory.
- Acting on business objects — that is [RFC 023](./023-closed-loop-actions.md).
- Full bidirectional sync of arbitrary note bodies to the cloud. RFC 036 permits encrypted, purpose-bound relationship evidence under its retention and authorization policy.
...
- **Backend as the canonical store for every knowledge entity** — rejected: breaks local-first. The vault remains canonical for user-authored notes. Customer relationships use the narrower RFC 036 federated model because team-visible corrections, approvals, and cross-client parity require shared authority.
