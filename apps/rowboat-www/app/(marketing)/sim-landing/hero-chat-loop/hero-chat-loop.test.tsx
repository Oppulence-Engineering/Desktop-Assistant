...
vi.mock("./hero-chat-welcome", () => ({
  HeroChatWelcome: () => null,
}));
vi.mock("./sidebar-hotspots", () => ({
  HERO_TOOLTIP_OFFSET: 8,
}));
vi.mock(
  "@/sim-port/workspace/home/components/message-content/components/agent-group/agent-group-view",
  () => ({
...
vi.mock("./hero-chat-loop/hero-tool-call-item", () => ({
  HeroToolCallItem: () => null,
}));
vi.mock(
  "@/sim-port/workspace/home/components/message-content/components/special-tags/pending-tag-indicator",
  () => ({
...
vi.mock("@/sim-port/workspace/home/components/message-content/components/question", () => ({
  parseQuestionAnswerMessage: () => undefined,
  QuestionDisplay: () => <p>What would you like to do next?</p>,
}));
vi.mock(
  "@/sim-port/workspace/home/components/user-input/components/send-button/send-button",
  () => ({ SendButton: () => null }),
);
...
function hasExecutingTool(items: AgentGroupItem[]): boolean {
  return items.some(
    (item) => item.type === "tool" && item.data.status === ToolCallStatus.executing,
  );
}
...
describe("HeroChatLoop production thinking handoff", () => {
  ...
  it("shows completed agent groups and the response after building finishes", () => {
    ...
    expect(groups.every((group) => !group.isStreaming && !hasExecutingTool(group.items))).toBe(
      true,
    );
