...
import styles from "./production-workflow-stage.module.css";
...
function ProductionWorkflowCanvas({
  ...
  /**
   * The homepage resource pane starts at width 0, and the hero demo can
   * measure 0×0 on the first paint. React Flow logs error #004 unless its
   * parent already has a box, so the graph waits for a real size.
   */
  const [canMountFlow, setCanMountFlow] = useState(false);
  /**
  ...
  useEffect(() => {
    ...
    const syncMount = () => {
      if (container.clientWidth > 0 && container.clientHeight > 0) {
        setCanMountFlow(true);
      }
    };

    syncMount();
    if (typeof ResizeObserver === "undefined") return;
    const observer = new ResizeObserver(syncMount);
