...
import {
  DevHealthErrorResponseSchema,
  DevHealthResponseSchema,
  type DevHealthResponse,
} from "@/lib/dev/dev-health";
import { readDevEnvFromProcess } from "@/lib/dev/validate-env";

/** Dev-only upstream health for the toolkit API tab. */
export async function GET() {
  if (process.env.NODE_ENV === "production") {
    return NextResponse.json(DevHealthErrorResponseSchema.parse({ error: "not found" }), {
      status: 404,
    });
  }
  ...
  let api: DevHealthResponse["api"] = { ok: false };
