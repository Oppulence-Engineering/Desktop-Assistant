...
Fly.io is the production-only deployment target for `rowboat-www`. There is no Fly staging app.
...
```bash
fly apps create oppulence-rowboat-www --org playbookmedia-llc
...
fly deploy . \
  ...
  --remote-only
fly scale count 2 --app oppulence-rowboat-www --region iad,sjc --max-per-region 1 --yes
```
...
Pushes to `main` and manual dispatches run `Deploy rowboat-www to Fly.io`.
Configure the GitHub `production` environment with:

- `ROWBOAT_WWW_FLY_API_TOKEN` secret: an app-scoped deploy token from
...
- Optional `ROWBOAT_WWW_FLY_APP_NAME` variable when the Fly App is not named `oppulence-rowboat-www`.
...
monorepo root, enforces one Machine in `iad` and one in `sjc`, checks Fly health, and runs the smoke test.
