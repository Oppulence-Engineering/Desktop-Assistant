/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */
import type { RelationshipObservationNormalizedFacts } from "./relationshipObservationNormalizedFacts";

/**
 * Immutable, idempotent provider evidence used to project relationship state.
 */
export interface RelationshipObservation {
  /** Hash of summary, facts, and sealed payload. */
  contentHash: string;
  /** Normalized event type. */
  eventType: string;
  /** Provider event id. */
  externalId: string;
  /** Stable UUID primary key. */
  id: string;
  /** Provider-neutral normalized facts. */
  normalizedFacts: RelationshipObservationNormalizedFacts;
  /** Provider occurrence time. */
  occurredAt: string;
  /** Ingestion time. */
  receivedAt: string;
  /** Evidence source. */
  source: string;
  /** Provider account id. */
  sourceAccountId?: string;
  /** Provider event version. */
  sourceVersion: string;
  /** Bounded evidence summary. */
  summary?: string;
}
