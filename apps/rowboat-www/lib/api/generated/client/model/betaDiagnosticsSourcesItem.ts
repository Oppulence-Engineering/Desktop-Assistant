/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */

/**
 * Source diagnostic.
 */
export type BetaDiagnosticsSourcesItem = {
  /** Completed units. */
  backfillCompleted: number;
  /** Backfill phase. */
  backfillPhase: string;
  /** Total units. */
  backfillTotal: number;
  /** Completeness state. */
  completeness: string;
  /** One-way connection support reference. */
  connectionRef: string;
  /** Safe categorical error. */
  errorCode?: string;
  /** Current lag. */
  lagSeconds: number;
  /** Count only; scope values remain on the user-facing connection card. */
  missingScopeCount: number;
  /** Retry attempts. */
  retryCount: number;
  /** Provider. */
  source: string;
  /** One-way provider account support reference. */
  sourceAccountRef: string;
  /** Lifecycle state. */
  status: string;
};
