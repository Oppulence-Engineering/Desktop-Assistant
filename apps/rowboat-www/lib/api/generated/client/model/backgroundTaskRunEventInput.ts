/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */
import type { BackgroundTaskRunEventInputEvent } from "./backgroundTaskRunEventInputEvent";

/**
 * One event to append to a run log mirror.
 */
export interface BackgroundTaskRunEventInput {
  /** Original JSON event object from the desktop run log. */
  event: BackgroundTaskRunEventInputEvent;
  /** Zero-based sequence number within the run log. */
  seq: number;
  /**
   * Optional event type. If omitted, rowboat-api reads event.type when present.
   * @nullable
   */
  type?: string | null;
}
