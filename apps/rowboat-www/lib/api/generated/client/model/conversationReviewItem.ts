/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */
import type { ConversationReviewItemBefore } from "./conversationReviewItemBefore";
import type { ConversationReviewItemKind } from "./conversationReviewItemKind";
import type { ConversationReviewItemProposedAfter } from "./conversationReviewItemProposedAfter";
import type { ConversationReviewItemStatus } from "./conversationReviewItemStatus";

/**
 * One evidence-backed proposed change requiring approve, correct, reject, or defer review.
 */
export interface ConversationReviewItem {
  /** Pinned relationship-state version. */
  baselineVersion?: number;
  /** Idempotent review batch id. */
  batchId?: string;
  /** State pinned before conversation processing. */
  before?: ConversationReviewItemBefore;
  /** Extraction and capture caveats. */
  caveats?: string[];
  /** Material claim id. */
  claimId?: string;
  /** Current confidence. */
  confidence: number;
  /** Current inferred value. */
  currentValue: string;
  /** Actions invalidated by rejection or correction. */
  dependentActionIds?: string[];
  /** Exact words under review. */
  exactQuote?: string;
  /** Stable UUID primary key. */
  id: string;
  /** Review kind. */
  kind: ConversationReviewItemKind;
  /** Review prompt. */
  label: string;
  /** Supporting observation. */
  observationId: string;
  /** Typed proposed value after this item. */
  proposedAfter?: ConversationReviewItemProposedAfter;
  /** Canonical state dimension affected by correction. */
  stateDimension?: string;
  /** Lifecycle/status slug. Subscription rows use billing states; background task runs use queued/running/succeeded/failed/stopped. */
  status?: ConversationReviewItemStatus;
}
