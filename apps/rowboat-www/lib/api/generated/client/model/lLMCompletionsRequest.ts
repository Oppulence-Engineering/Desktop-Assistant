/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */

/**
 * OpenAI-compatible legacy completions request.
 */
export interface LLMCompletionsRequest {
  /** Maximum output tokens to reserve and request. */
  max_tokens?: number;
  /** Desktop-facing LLM model id. */
  model: string;
  /** Prompt string or prompt array forwarded to the upstream. */
  prompt: unknown;
  /** When true, stream upstream completion events. */
  stream?: boolean;
  /** Sampling temperature forwarded to the upstream. */
  temperature?: number;
  [key: string]: unknown;
}
