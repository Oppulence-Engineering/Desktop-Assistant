/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */
import type { RelationshipStateSnapshotState } from "./relationshipStateSnapshotState";

/**
 * Immutable projection snapshot created only when material relationship state changes.
 */
export interface RelationshipStateSnapshot {
  /** Assertions selected by deterministic precedence. */
  assertionIds: string[];
  /** Material dimensions that changed. */
  changedDimensions: string[];
  /** Snapshot creation time. */
  createdAt: string;
  /** Explicit evaluation time used by the projector. */
  evaluatedAt: string;
  /** Stable UUID primary key. */
  id: string;
  /** Projector version used for this snapshot. */
  projectorVersion: number;
  /** Opaque one-time OAuth state/session ticket. */
  state: RelationshipStateSnapshotState;
  /** Stable hash of canonical state and winning assertions. */
  stateHash: string;
  /** Relationship state version. */
  version: number;
}
