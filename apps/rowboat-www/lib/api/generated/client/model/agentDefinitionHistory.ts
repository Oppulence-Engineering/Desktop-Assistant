/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */
import type { AgentDefinitionHistoryOperation } from "./agentDefinitionHistoryOperation";

export interface AgentDefinitionHistory {
  agent_sync_error?: string;
  agent_sync_state: string;
  channel_bindings?: string;
  connector_reqs?: string[];
  content_hash?: string;
  /** Row creation timestamp. */
  created_at: string;
  enabled_tools?: string[];
  forked_from?: string;
  /** Timestamp when this history record was written. */
  history_time: string;
  /** Stable UUID primary key. */
  id: string;
  instructions?: string;
  limits_json?: string;
  managed_by: string;
  /** Desktop-facing LLM model id. */
  model?: string;
  name: string;
  /** Mutation operation that produced this history row. */
  operation: AgentDefinitionHistoryOperation;
  /** Provider slug. Depending on the row this may be an OAuth provider, LLM provider, or execution backend. */
  provider?: string;
  raw_source?: string;
  /** UUID of the source row represented by a history row. */
  ref?: string;
  revision: number;
  slug: string;
  source: string;
  source_format: string;
  subagent_refs?: string[];
  tools_json?: string;
  /** Last row update timestamp. */
  updated_at: string;
}
