/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Solomon AI API
 * Solomon AI's desktop API. The API brokers WorkOS sign-in, billing and credit state, OpenAI-compatible LLM calls, vendor proxies, Google OAuth handoff, connector OAuth, internal webhooks, and admin GraphQL. The ent-generated entity models remain in components as schema references; the documented paths below are the routes mounted by cmd/server/wire.go.
 * OpenAPI spec version: 0.1.0
 */
import { faker } from "@faker-js/faker";

import type { MeResponse } from "../model";

export const getGetMeResponseMock = (
  overrideResponse: Partial<Extract<MeResponse, object>> = {},
): MeResponse => ({
  billing: {
    plan: faker.helpers.arrayElement([
      faker.helpers.arrayElement(["free", "starter", "pro", "intelligence"] as const),
      null,
    ]),
    status: faker.helpers.arrayElement([
      faker.helpers.arrayElement(["active", "trialing", "past_due", "canceled"] as const),
      null,
    ]),
    trialExpiresAt: faker.helpers.arrayElement([
      faker.string.alpha({ length: { min: 10, max: 20 } }),
      null,
    ]),
    usage: {
      availableCredits: faker.number.int(),
      daily: {
        availableCredits: faker.number.int(),
        sanctionedCredits: faker.number.int(),
        usageDay: faker.string.alpha({ length: { min: 10, max: 20 } }),
        usedCredits: faker.number.int(),
      },
      monthly: {
        availableCredits: faker.number.int(),
        sanctionedCredits: faker.number.int(),
        usedCredits: faker.number.int(),
      },
      sanctionedCredits: faker.number.int(),
      usedCredits: faker.number.int(),
    },
  },
  user: { email: faker.string.alpha({ length: { min: 10, max: 20 } }), id: faker.string.uuid() },
  ...overrideResponse,
});
