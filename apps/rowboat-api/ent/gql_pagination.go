...
// ConnectorCredentialCleanupJobEdge is the edge representation of ConnectorCredentialCleanupJob.
type ConnectorCredentialCleanupJobEdge struct {
	Node   *ConnectorCredentialCleanupJob `json:"node"`
	Cursor Cursor                         `json:"cursor"`
}

// ConnectorCredentialCleanupJobConnection is the connection containing edges to ConnectorCredentialCleanupJob.
type ConnectorCredentialCleanupJobConnection struct {
	Edges      []*ConnectorCredentialCleanupJobEdge `json:"edges"`
	PageInfo   PageInfo                             `json:"pageInfo"`
	TotalCount int                                  `json:"totalCount"`
}

func (c *ConnectorCredentialCleanupJobConnection) build(nodes []*ConnectorCredentialCleanupJob, pager *connectorcredentialcleanupjobPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	var nodeAt func(int) *ConnectorCredentialCleanupJob
	if last != nil {
		n := len(nodes) - 1
		nodeAt = func(i int) *ConnectorCredentialCleanupJob {
			return nodes[n-i]
		}
	} else {
		nodeAt = func(i int) *ConnectorCredentialCleanupJob {
	...
	c.Edges = make([]*ConnectorCredentialCleanupJobEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &ConnectorCredentialCleanupJobEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
// ConnectorCredentialCleanupJobPaginateOption enables pagination customization.
type ConnectorCredentialCleanupJobPaginateOption func(*connectorcredentialcleanupjobPager) error

// WithConnectorCredentialCleanupJobOrder configures pagination ordering.
func WithConnectorCredentialCleanupJobOrder(order *ConnectorCredentialCleanupJobOrder) ConnectorCredentialCleanupJobPaginateOption {
	if order == nil {
		order = DefaultConnectorCredentialCleanupJobOrder
	}
	o := *order
	return func(pager *connectorcredentialcleanupjobPager) error {
		if err := o.Direction.Validate(); err != nil {
			return err
		}
		if o.Field == nil {
			o.Field = DefaultConnectorCredentialCleanupJobOrder.Field
		}
		pager.order = &o
		return nil
	}
...
// WithConnectorCredentialCleanupJobFilter configures pagination filter.
func WithConnectorCredentialCleanupJobFilter(filter func(*ConnectorCredentialCleanupJobQuery) (*ConnectorCredentialCleanupJobQuery, error)) ConnectorCredentialCleanupJobPaginateOption {
	return func(pager *connectorcredentialcleanupjobPager) error {
		if filter == nil {
			return errors.New("ConnectorCredentialCleanupJobQuery filter cannot be nil")
		}
		pager.filter = filter
		return nil
	}
...
type connectorcredentialcleanupjobPager struct {
	reverse bool
	order   *ConnectorCredentialCleanupJobOrder
	filter  func(*ConnectorCredentialCleanupJobQuery) (*ConnectorCredentialCleanupJobQuery, error)
}

func newConnectorCredentialCleanupJobPager(opts []ConnectorCredentialCleanupJobPaginateOption, reverse bool) (*connectorcredentialcleanupjobPager, error) {
	pager := &connectorcredentialcleanupjobPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
	...
	if pager.order == nil {
		pager.order = DefaultConnectorCredentialCleanupJobOrder
	}
	return pager, nil
}

func (p *connectorcredentialcleanupjobPager) applyFilter(query *ConnectorCredentialCleanupJobQuery) (*ConnectorCredentialCleanupJobQuery, error) {
	if p.filter != nil {
		return p.filter(query)
	}
	return query, nil
...
func (p *connectorcredentialcleanupjobPager) toCursor(_m *ConnectorCredentialCleanupJob) Cursor {
	return p.order.Field.toCursor(_m)
}

func (p *connectorcredentialcleanupjobPager) applyCursors(query *ConnectorCredentialCleanupJobQuery, after, before *Cursor) (*ConnectorCredentialCleanupJobQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultConnectorCredentialCleanupJobOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *connectorcredentialcleanupjobPager) applyOrder(query *ConnectorCredentialCleanupJobQuery) *ConnectorCredentialCleanupJobQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	query = query.Order(p.order.Field.toTerm(direction.OrderTermOption()))
	if p.order.Field != DefaultConnectorCredentialCleanupJobOrder.Field {
		query = query.Order(DefaultConnectorCredentialCleanupJobOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *connectorcredentialcleanupjobPager) orderExpr(query *ConnectorCredentialCleanupJobQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultConnectorCredentialCleanupJobOrder.Field {
			b.Comma().Ident(DefaultConnectorCredentialCleanupJobOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}

// Paginate executes the query and returns a relay based cursor connection to ConnectorCredentialCleanupJob.
func (_m *ConnectorCredentialCleanupJobQuery) Paginate(
	ctx context.Context, after *Cursor, first *int,
	before *Cursor, last *int, opts ...ConnectorCredentialCleanupJobPaginateOption,
) (*ConnectorCredentialCleanupJobConnection, error) {
	if err := validateFirstLast(first, last); err != nil {
		return nil, err
	}
	pager, err := newConnectorCredentialCleanupJobPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
	if _m, err = pager.applyFilter(_m); err != nil {
	...
	conn := &ConnectorCredentialCleanupJobConnection{Edges: []*ConnectorCredentialCleanupJobEdge{}}
	ignoredEdges := !hasCollectedField(ctx, edgesField)
	if hasCollectedField(ctx, totalCountField) || hasCollectedField(ctx, pageInfoField) {
		hasPagination := after != nil || first != nil || before != nil || last != nil
		if hasPagination || ignoredEdges {
...
// ConnectorCredentialCleanupJobOrderField defines the ordering field of ConnectorCredentialCleanupJob.
type ConnectorCredentialCleanupJobOrderField struct {
	// Value extracts the ordering value from the given ConnectorCredentialCleanupJob.
	Value    func(*ConnectorCredentialCleanupJob) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) connectorcredentialcleanupjob.OrderOption
	toCursor func(*ConnectorCredentialCleanupJob) Cursor
}

// ConnectorCredentialCleanupJobOrder defines the ordering of ConnectorCredentialCleanupJob.
type ConnectorCredentialCleanupJobOrder struct {
	Direction OrderDirection                           `json:"direction"`
	Field     *ConnectorCredentialCleanupJobOrderField `json:"field"`
}

// DefaultConnectorCredentialCleanupJobOrder is the default ordering of ConnectorCredentialCleanupJob.
var DefaultConnectorCredentialCleanupJobOrder = &ConnectorCredentialCleanupJobOrder{
	Direction: entgql.OrderDirectionAsc,
	Field: &ConnectorCredentialCleanupJobOrderField{
		Value: func(_m *ConnectorCredentialCleanupJob) (ent.Value, error) {
			return _m.ID, nil
		},
		column: connectorcredentialcleanupjob.FieldID,
		toTerm: connectorcredentialcleanupjob.ByID,
		toCursor: func(_m *ConnectorCredentialCleanupJob) Cursor {
			return Cursor{ID: _m.ID}
		},
	},
}

// ToEdge converts ConnectorCredentialCleanupJob into ConnectorCredentialCleanupJobEdge.
func (_m *ConnectorCredentialCleanupJob) ToEdge(order *ConnectorCredentialCleanupJobOrder) *ConnectorCredentialCleanupJobEdge {
	if order == nil {
		order = DefaultConnectorCredentialCleanupJobOrder
	}
	return &ConnectorCredentialCleanupJobEdge{
		Node:   _m,
		Cursor: order.Field.toCursor(_m),
	}
}
