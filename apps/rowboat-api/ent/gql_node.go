...
var connectorcredentialcleanupjobImplementors = []string{"ConnectorCredentialCleanupJob", "Node"}

// IsNode implements the Node interface check for GQLGen.
func (*ConnectorCredentialCleanupJob) IsNode() {}
...
func (c *Client) noder(ctx context.Context, table string, id uuid.UUID) (Noder, error) {
	...
	case commitmentevent.Table:
		...
		return query.Only(ctx)
	case connectorcredentialcleanupjob.Table:
		query := c.ConnectorCredentialCleanupJob.Query().
			Where(connectorcredentialcleanupjob.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, connectorcredentialcleanupjobImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []uuid.UUID) ([]Noder, error) {
	...
	case connectorcredentialcleanupjob.Table:
		query := c.ConnectorCredentialCleanupJob.Query().
			Where(connectorcredentialcleanupjob.IDIn(ids...))
		query, err := query.CollectFields(ctx, connectorcredentialcleanupjobImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
