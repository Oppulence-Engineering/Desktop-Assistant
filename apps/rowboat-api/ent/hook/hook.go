...
// The ComposioAccountFunc type is an adapter to allow the use of ordinary
// function as ComposioAccount mutator.
type ComposioAccountFunc func(context.Context, *ent.ComposioAccountMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ComposioAccountFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ComposioAccountMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ComposioAccountMutation", m)
}
