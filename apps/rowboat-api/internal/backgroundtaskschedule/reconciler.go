...
func (r *Reconciler) reconcileTask(ctx context.Context, task *ent.BackgroundTask, log *zap.Logger) string {
	...
	case outcomeManaged:
		...
		if err != nil {
			...
			r.Syncer.writeState(ctx, task, "failed", err.Error(), log)
			return scheduleID
		}
		drift := ""
		switch {
		...
		if drift != "" {
			// Re-read the row before mutating Temporal: this pass's snapshot
			// may predate a concurrent handler patch, and "repairing" from a
			// stale snapshot would revert the user's fresh schedule to the
			// old cron until the next pass.
			fresh, freshDesired, ok := r.refreshDesired(ctx, task, log)
			if !ok {
				return scheduleID
			}
		...
		r.Syncer.writeState(ctx, task, "current", "", log)
		return scheduleID
	...
	case outcomeInvalid:
		...
		r.Syncer.writeState(ctx, task, "failed", "invalid cron expression "+cronExpr, log)
		return ""

	case outcomePausedKeep:
		// Ensure exists-but-paused for fast unpause.
		desired := Desired(userID, task, cronExpr, r.Syncer.Cfg, true)
		...
		r.Syncer.writeState(ctx, task, "paused", "", log)
		return scheduleID
...
// refreshDesired re-reads the task immediately before a Temporal mutation and
// recomputes its desired state, so the reconciler never acts on a snapshot a
// concurrent handler write has overtaken. ok=false means the task vanished or
// no longer wants a managed schedule — skip; the next pass (or the sweep)
// converges it.
func (r *Reconciler) refreshDesired(ctx context.Context, task *ent.BackgroundTask, log *zap.Logger) (*ent.BackgroundTask, DesiredCronSchedule, bool) {
	...
	if err != nil {
		...
		return nil, DesiredCronSchedule{}, false
	}
	o, cronExpr := classify(fresh)
	if o != outcomeManaged || fresh.Edges.User == nil {
		return nil, DesiredCronSchedule{}, false
	}
	return fresh, Desired(fresh.Edges.User.ID.String(), fresh, cronExpr, r.Syncer.Cfg, false), true
}
