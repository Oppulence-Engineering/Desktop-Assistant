...
func (r *DefaultRuntime) Execute(ctx context.Context, in RunInput) (RunOutput, error) {
	...
	for call := 0; call < in.Limits.MaxLLMCalls; call++ {
		...
		if err := r.emit(runCtx, in, eventLLMCallStarted, map[string]any{
			"provider": providerOf(in.Model), "model": in.Model, "callIndex": call,
		...
		if err != nil {
			if derr := r.checkDeadline(ctx, runCtx, in); derr != nil {
				return out, derr // the LLM call died on OUR deadline
			}
			return out, err // classified by the adapter (llm_call_failed)
		}
		out.LLMCalls++
		...
		if len(turn.Message.ToolCalls) == 0 {
			return r.finalize(runCtx, in, staged, out, turn.Message.Content)
		}
		...
		for _, callReq := range turn.Message.ToolCalls {
			...
			if ierr != nil {
				...
				// Classified failures (connector_unavailable, …) are terminal;
				// plain implementation errors are surfaced to the model.
				var re *RuntimeError
			...
			transcript = appendToolResult(transcript, callReq.ID, truncate(string(result), toolResultCap))
		}
	}
...
func (r *DefaultRuntime) finalize(ctx context.Context, in RunInput, staged *stagedArtifact, out RunOutput, finalContent string) (RunOutput, error) {
	...
	if err != nil {
		var re *RuntimeError
		if errors.As(err, &re) {
			return out, r.limitBreached(ctx, in, re.Code, "artifact_bytes", bytes, in.Limits.MaxArtifactBytes)
		}
		return out, err
	}
