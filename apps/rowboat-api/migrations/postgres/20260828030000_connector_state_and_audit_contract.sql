-- RFC 012 contract phase. Deploy only after the hash-aware Google, Slack, and
-- connector binaries have been live for at least the maximum pending-state TTL
-- plus clock skew. Those binaries hash callback/claim input and retain a legacy
-- raw-state read fallback, so every unexpired pre-rollout row remains consumable
-- before and after this migration rewrites it. The statements are idempotent so
-- a partially completed operational rollout can be retried safely.
