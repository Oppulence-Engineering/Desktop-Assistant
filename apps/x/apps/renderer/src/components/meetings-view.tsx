...
function formatEventTimeRangeCompact(event: UpcomingEvent): string {
  if (event.isAllDay) return "All day";
  const startStr = event.start.toLocaleTimeString([], {
    hour: "numeric",
    minute: "2-digit",
  });
  ...
  const endStr = event.end.toLocaleTimeString([], {
    hour: "numeric",
    minute: "2-digit",
  });
    ...
    <div className="overflow-hidden rounded-none border bg-card">
      <div className="flex items-center justify-between gap-3 border-b bg-muted px-5 py-3.5">
        <div className="flex min-w-0 items-baseline gap-2">
          ...
          {isToday ? (
            <span className="shrink-0 rounded-none bg-foreground px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-background">
              Today
            </span>
          ) : null}
        </div>
          ...
          <div className="shrink-0">
            {event.conferenceLink ? (
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  triggerMeetingCapture(event, false);
                }}
                onMouseDown={(e) => e.stopPropagation()}
                className="inline-flex items-center gap-1.5 rounded-none border bg-background px-2.5 py-1.5 text-xs font-medium text-foreground transition-colors hover:bg-accent"
              >
...
function EventDetailsPopover({
  ...
  return (
    <PopoverContent
      ...
      side="bottom"
      sideOffset={6}
      className="w-[min(380px,calc(100vw-32px))] rounded-none p-0 shadow-xl"
      style={{
    ...
    >
      <div
        className="flex items-center justify-end gap-1 border-b px-3 py-2"
      ...
      >
        {event.htmlLink ? (
          <button
            ...
            className="inline-flex size-8 items-center justify-center rounded-none transition-colors"
      ...
      <div className="space-y-4 px-5 py-4">
        <div className="flex gap-3">
          ...
          <div className="min-w-0">
            <h4
              className="break-words text-[20px] font-normal leading-6"
              style={{ color: "var(--foreground, #09090b)" }}
            >
              {event.summary}
            </h4>
          </div>
        ...
        {attendees.length > 0 ? (
          <EventDetailRow
            ...
            value={
              attendees.slice(0, 8).join(", ") +
              (attendees.length > 8 ? `, +${attendees.length - 8} more` : "")
            }
          />
        ) : null}
          ...
          <div className="flex gap-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => handleMeetingCapture(false)}
              >
                Take notes only
              </Button>
            </div>
        ...
        {descriptionParts.length > 0 ? (
          <div className="flex gap-3">
            ...
            <div
              className="max-h-40 overflow-auto whitespace-pre-wrap break-words text-sm leading-5"
      ...
      {open && menuPos
        ? createPortal(
            <div
              ...
              className="min-w-36 overflow-hidden rounded-none border bg-popover text-popover-foreground shadow-lg"
            >
              <button
                type="button"
                onMouseDown={(e) => e.stopPropagation()}
