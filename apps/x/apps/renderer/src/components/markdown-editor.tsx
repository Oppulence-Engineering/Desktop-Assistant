...
interface MarkdownEditorProps {
  ...
  /** Select the title heading once on mount — used for freshly created untitled notes. */
...
const resolveWorkspaceLinkPath = (href: string, notePath?: string) => {
  ...
  if (decoded.startsWith("knowledge/") || !notePath)
    return collapseRelativeSegments(decoded.replace(/^\.\//, ""));
...
export const MarkdownEditor = forwardRef<MarkdownEditorHandle, MarkdownEditorProps>(
  function MarkdownEditor(
    ...
    const [anchorPosition, setAnchorPosition] = useState<{ left: number; top: number } | null>(
      null,
    );
    ...
    const wikiKeyStateRef = useRef<{ open: boolean; options: string[]; value: string }>({
      open: false,
      options: [],
      value: "",
    });
    const handleSelectWikiLinkRef = useRef<(path: string) => void>(() => {});
    const [activeRowboatMention, setActiveRowboatMention] = useState<RowboatMentionMatch | null>(
      null,
    );
    ...
    const [atAnchorPosition, setAtAnchorPosition] = useState<{ left: number; top: number } | null>(
      null,
    );
    const [atCommandValue, setAtCommandValue] = useState<string>("");
    const atKeyStateRef = useRef<{ open: boolean; options: string[]; value: string }>({
      open: false,
      options: [],
      value: "",
    });
    ...
    const preventTitleHeadingDemotion = useCallback(
      (view: EditorView, event: KeyboardEvent) => {
        ...
        const headingLevel =
          ($from.parent.attrs as { level?: number } | null | undefined)?.level ?? 0;
    ...
    const promoteFirstParagraphToTitleHeading = useCallback(
      (view: EditorView) => {
        ...
        editorProps: {
          ...
          handleKeyDown: (view, event) => {
            ...
            if (state.open) {
              ...
              if (event.key === "ArrowDown" || event.key === "ArrowUp") {
                ...
                const nextIndex =
                  (currentIndex + delta + state.options.length) % state.options.length;
              ...
              if (event.key === "Enter" || event.key === "Tab") {
                ...
                event.preventDefault();
                event.stopPropagation();
                const selected = state.options.includes(state.value)
                  ? state.value
                  : state.options[0];
            ...
            if (atState.open) {
              ...
              if (event.key === "ArrowDown" || event.key === "ArrowUp") {
                ...
                const delta = event.key === "ArrowDown" ? 1 : -1;
                const nextIndex =
                  (currentIndex + delta + atState.options.length) % atState.options.length;
              ...
              if (event.key === "Enter" || event.key === "Tab") {
                ...
                event.preventDefault();
                event.stopPropagation();
                const selected = atState.options.includes(atState.value)
                  ? atState.value
                  : atState.options[0];
          ...
          handleDOMEvents: {
            click: (view, event) => {
              ...
              event.preventDefault();
              void window.ipc
                .invoke("shell:openPath", { path: workspacePath })
                .then((result) => {
    ...
    // Fresh untitled notes: move focus from the New-note button into the title
    // and select the placeholder so typing renames the note. Without this the
    // button keeps focus — typed text is dropped and Enter creates more notes.
    // The doc hydrates via the content-sync effect below, not via a prop this
    // effect could depend on — so wait on the editor's own update stream.
    const autoFocusedTitleRef = useRef(false);
    useEffect(() => {
      if (!autoFocusTitle || !editable || !editor || autoFocusedTitleRef.current) return;
      const tryFocusTitle = () => {
        if (autoFocusedTitleRef.current) return true;
        const first = editor.state.doc.firstChild;
        if (!first || first.type.name !== "heading" || first.content.size === 0) return false;
        autoFocusedTitleRef.current = true;
        editor
          .chain()
      ...
      if (tryFocusTitle()) return;
      const onUpdate = () => {
        if (tryFocusTitle()) editor.off("update", onUpdate);
      };
      editor.on("update", onUpdate);
      return () => {
        editor.off("update", onUpdate);
      };
    }, [editor, autoFocusTitle, editable]);
        ...
        if (normalizeForCompare(currentContent) !== normalizeForCompare(content)) {
          ...
          if (hadFocus) {
            ...
            try {
              ...
              /* selection no longer valid in the new doc — ignore */
            }
          }
