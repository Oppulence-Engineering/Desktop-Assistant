...
export function VersionHistoryPanel({
  ...
  const relPath = path.startsWith("knowledge/")
  ...
  const loadHistory = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await window.ipc.invoke("knowledge:history", {
        path: relPath,
      });
  ...
  const handleSelectCommit = useCallback(
    async (oid: string, isLatest: boolean) => {
      if (isLatest) {
        setSelectedOid(null);
      ...
      setSelectedOid(oid);
      try {
        const result = await window.ipc.invoke("knowledge:fileAtCommit", {
          path: relPath,
          oid,
        });
    ...
    <div className="flex flex-col w-[280px] shrink-0 border-l border-border bg-background">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border shrink-0">
        ...
        <button
          type="button"
          onClick={onClose}
          className="flex h-6 w-6 items-center justify-center rounded-none text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
