...
function isReplyQuoteBoundary(lines: string[], index: number): boolean {
  ...
  if (/^From:\s+\S/i.test(line)) {
    ...
    return (
      next.some((value) => /^(Sent|Date):\s+\S/i.test(value)) &&
...
function buildRecipients(
  ...
  if (
    ...
    to.length === 0 &&
    self &&
    rawTo.some((token) => extractAddress(token).toLowerCase() === self)
  ) {
    to.push(self);
  }
...
function isStyledHtml(html: string): boolean {
  ...
  doc
    .querySelectorAll('.gmail_quote, .gmail_attr, blockquote[type="cite"]')
    .forEach((n) => n.remove());
  ...
  return (
    <>
      <div className="gmail-message-plain">
        ...
        {quoted && showQuote && (
          <pre className="gmail-message-pre gmail-message-pre-quoted">{quoted}</pre>
        )}
      </div>
      {quoted && (
        <button
  ...
  const handleLoad = useCallback(() => {
    ...
    const measure = () => {
      ...
      saveTimerRef.current = setTimeout(() => {
        ...
        void window.ipc
          .invoke("gmail:saveMessageHeight", {
            threadId,
...
function MessageAttachments({
  ...
  attachments: NonNullable<GmailThreadMessage["attachments"]>;
}) {
    ...
    <div className="gmail-recipient-row">
      ...
      <div className="gmail-recipient-field">
        {value.map((token, index) => (
          <span
            key={`${token}-${index}`}
            className="gmail-recipient-chip"
...
function ComposeBox({
  ...
  const refineWithCopilot = () => {
    ...
    const lines: string[] = [];
    lines.push(
      `Help me refine this draft email response. **Please ask me how I want to refine it before making any changes** — wait for my answer, then apply the edits.`,
    );
    lines.push("");
  ...
  return (
    <div className="gmail-compose-card">
      <div className="gmail-compose-header">
        <span>{modeLabel}</span>
        <button type="button" onClick={onClose} aria-label="Close compose">
          ×
        </button>
      </div>
      <RecipientField
      ...
      {linkOpen && (
        <div className="gmail-compose-link-popover" onMouseDown={(event) => event.preventDefault()}>
          ...
          />
          <button type="button" className="gmail-compose-link-popover-apply" onClick={applyLink}>
            Apply
          </button>
          <button type="button" className="gmail-compose-link-popover-cancel" onClick={cancelLink}>
            Cancel
          </button>
        </div>
      )}
      <div className="gmail-compose-actions">
        ...
        <button type="button" className="gmail-compose-link" onClick={onClose}>
          Discard
        </button>
      </div>
    </div>
...
function ThreadDetail({
  ...
  useEffect(() => {
    let cancelled = false;
    window.ipc
      .invoke("gmail:getAccountEmail", {})
      .then((res) => {
        if (!cancelled && res?.email) setSelfEmail(res.email);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
          ...
          {thread.messages.map((message, index) => {
            ...
            return (
              <div
                ...
                className={cn("gmail-message", isExpanded && "gmail-message-expanded")}
              >
                <div
                  className="gmail-message-avatar"
                  style={{ backgroundColor: avatarColor(message.from) }}
                >
                ...
                <div className="gmail-message-main">
                  ...
                  {isExpanded && <MessageBody message={message} threadId={thread.threadId} />}
                </div>
              </div>
            );
          })}
...
export function EmailView({ initialThreadId, threadIdVersion }: EmailViewProps = {}) {
  const [important, setImportant] = useState<SectionState>(() =>
    clearLoadingFlag(persistedImportant),
  );
  ...
  useEffect(() => {
    ...
    void check();
    const cleanupOAuthConnect = window.ipc.on("oauth:didConnect", () => {
      void check();
    });
    return () => {
      cancelled = true;
  ...
  const setSection = useCallback(
    (section: InboxSection, updater: (prev: SectionState) => SectionState) => {
  ...
  const updateThreadInState = useCallback(
    (threadId: string, updater: (t: GmailThread) => GmailThread) => {
  ...
  const markThreadReadAction = useCallback(
    async (threadId: string) => {
      ...
      // E14: revert the optimistic read state and toast on failure, mirroring
      // the archive/trash handlers (rather than only console.warn).
      const revertUnread = () =>
        updateThreadInState(threadId, (t) => ({
          ...t,
          unread: true,
      ...
      try {
        ...
        if (!result.ok) {
          revertUnread();
          toast(`Mark as read failed: ${result.error ?? "unknown error"}`, "error");
        }
      } catch (err) {
        revertUnread();
        toast(`Mark as read failed: ${err instanceof Error ? err.message : String(err)}`, "error");
      }
    },
    [updateThreadInState],
  );
  ...
  const trashThreadAction = useCallback(
    async (threadId: string) => {
      try {
  ...
  const sectionChannel = (section: InboxSection) =>
    section === "important"
      ? ("gmail:getImportant" as const)
      : ("gmail:getEverythingElse" as const);
  ...
  const reloadFirstPage = useCallback(
    async (section: InboxSection, options: { silent?: boolean } = {}) => {
      const epoch = ++epochsRef.current[section];
      if (options.silent) {
        setSection(section, (prev) => ({ ...prev, loadingPage: true }));
      } else {
    ...
    const cleanup = window.ipc.on("workspace:didChange", (event) => {
      ...
      switch (event.type) {
        ...
        case "deleted":
          ...
          break;
        case "moved":
          if ((event.from && matches(event.from)) || (event.to && matches(event.to)))
            triggerLiveReload();
          break;
        case "bulkChanged":
  ...
  const filterThreads = useCallback(
    (threads: GmailThread[]) => {
      ...
      return threads.filter((thread) => {
        const latest = latestMessage(thread);
        return [thread.subject, latest?.from, latest?.to, latest?.body].some((value) =>
  ...
  const visibleImportant = useMemo(
    () => filterThreads(important.threads),
    [important.threads, filterThreads],
  );
  ...
  const needsEmailReconnect =
    emailConnection?.connected === true && !emailConnection.hasRequiredScope;
      ...
      <div key={thread.threadId} className="gmail-row-group">
        ...
        >
          <button
            ...
            className={cn(
              "gmail-row",
              isSelected && "gmail-row-selected",
              isUnread && "gmail-row-unread",
            )}
          ...
          >
            ...
            <span className="gmail-row-content">
              ...
              <span>
                {thread.summary ? thread.subject : snippet(latest?.body || thread.latest_email)}
              </span>
            </span>
          ...
          <div className="gmail-row-actions" onMouseDown={stop} onClick={stop}>
            ...
            <button
              ...
              onClick={(e) => {
                stop(e);
                void archiveThreadAction(thread.threadId);
              }}
            >
              <Archive size={15} />
            </button>
            <button
              ...
              onClick={(e) => {
                stop(e);
                void trashThreadAction(thread.threadId);
              }}
            >
              <Trash2 size={15} />
            </button>
        ...
        {error && !hasAny ? (
            ...
            {important.threads.length > 0 && (
              <section className="gmail-section">
                <div className="gmail-list-header">
                  <span>Important</span>
                  <span>
                    {important.threads.length}
                    {important.hasReachedEnd ? "" : "+"} thread
                    {important.threads.length === 1 ? "" : "s"}
                  </span>
                </div>
            ...
            {important.hasReachedEnd && other.threads.length > 0 && (
              <section className="gmail-section">
                <div className="gmail-list-header">
                  ...
                  <span>
                    {other.threads.length}
                    {other.hasReachedEnd ? "" : "+"} thread{other.threads.length === 1 ? "" : "s"}
                  </span>
                </div>
          ...
          <div className="gmail-empty-state">
            {initialLoading
              ...
              : "No Gmail threads in your inbox cache yet."}
          </div>
        )}
      </div>
