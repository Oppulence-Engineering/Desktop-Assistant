...
function ImageAttachmentPreview({
  ...
  return (
    <img
      ...
      className="h-44 w-auto max-w-[300px] rounded-none border border-border/70 bg-muted object-cover"
      onError={() => {
        ...
        <div className="flex flex-wrap justify-end gap-2">
          {fileAttachments.map((attachment, index) => {
            ...
            return (
              <span
                key={`${attachment.path}-${index}`}
                className="inline-flex min-w-[240px] max-w-[440px] items-center gap-3 rounded-none border border-border/50 bg-muted/75 px-3 py-2.5 text-sm text-foreground"
