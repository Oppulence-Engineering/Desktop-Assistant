...
function toolKindIcon(kind?: string) {
  switch (kind) {
    ...
    case "edit":
      return <Pencil className="size-3.5 shrink-0 text-muted-foreground" />;
    case "delete":
...
function planMarker(status?: string) {
  ...
  if (status === "in_progress")
    return <CircleDot className="size-3.5 shrink-0 text-blue-500" />;
      ...
      {rows.map((row) => {
            ...
            <div key={row.id} className="flex flex-col gap-1">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="truncate text-foreground/90">
                  {row.title ?? row.toolKind ?? "Tool call"}
                </span>
              </div>
              {row.diffs.length > 0 && (
        ...
        if (row.kind === "plan") {
          return (
            <div
              key={row.id}
              className="flex flex-col gap-1 rounded-none border bg-muted/30 p-2"
            >
        ...
        // resolved permission
        const denied =
          row.decision === "reject" || row.decision === "cancelled";
        return (
          <div
            key={row.id}
            className={cn(
              "flex items-center gap-2 text-xs",
              denied ? "text-red-600" : "text-green-600",
            )}
          >
            {denied ? "✕" : "✓"}
            <span className="truncate">
              {denied ? "Denied" : "Allowed"}: {row.title}
            </span>
          </div>
        );
      })}
    ...
    <>
      ...
      {item.pendingCodePermission && (
        <CodeRunPermissionRequest
          ask={item.pendingCodePermission.ask}
