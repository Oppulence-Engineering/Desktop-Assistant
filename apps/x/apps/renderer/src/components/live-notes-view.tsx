...
export function LiveNotesView({
    ...
    try {
      ...
      const enriched = await Promise.all(
        result.notes.map(async (n) => {
          ...
          try {
            const detail = await window.ipc.invoke("live-note:get", {
              filePath: knowledgeRel,
            });
  ...
  useEffect(() => {
    ...
    const cleanupWorkspace = window.ipc.on("workspace:didChange", (event) => {
      switch (event.type) {
        ...
        case "moved":
          if (
            isKnowledgeMarkdownPath(event.from) ||
            isKnowledgeMarkdownPath(event.to)
          ) {
            scheduleReload();
          }
          break;
  ...
  const handleToggleState = useCallback(
    async (note: LiveNoteRow, active: boolean) => {
      ...
      try {
          ...
          prev.map((entry) =>
            entry.path === note.path
              ? {
                  ...
                  lastAttemptAt:
                    result.live!.lastAttemptAt ?? entry.lastAttemptAt,
                }
              : entry,
          ),
        );
        ...
        toast(
          err instanceof Error
            ? err.message
            : "Failed to update live-note state",
          "error",
        );
      } finally {
  ...
  const handleStop = useCallback(async (note: LiveNoteRow) => {
    ...
    try {
      const knowledgeRelative = note.path.replace(/^knowledge\//, "");
      const result = await window.ipc.invoke("live-note:stop", {
        filePath: knowledgeRelative,
      });
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="shrink-0 border-b border-border px-6 py-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Notes whose body is kept current by an agent. Toggle a note inactive
          to pause its agent.
        </p>
      </div>
      <div className="flex-1 overflow-auto p-6">
        {loading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-3 px-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">No live notes yet.</p>
          </div>
        ) : (
                        ...
                        <div className="flex min-w-0 flex-col gap-1">
                          <div className="flex items-center gap-1.5">
                            {hasError && (
                              ...
                              >
                                <title>
                                  Last run failed: {note.lastRunError}
                                </title>
                              </AlertCircle>
                            )}
                            <button
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <Switch
                              ...
                              onCheckedChange={(checked) => {
                                void handleToggleState(note, checked);
                              }}
                              disabled={isUpdating}
                            />
