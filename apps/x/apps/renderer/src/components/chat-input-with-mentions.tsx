...
function ChatInputInner({
  ...
  useEffect(() => {
    const load = () => {
      window.ipc
        .invoke("codeMode:getConfig", null)
  ...
  const loadCodingAgentFor = useCallback(
    async (dir: string | null): Promise<"claude" | "codex"> => {
  ...
  return (
    <div className="rowboat-chat-input rounded-none border border-border bg-background shadow-none">
        ...
        <div className="flex flex-wrap gap-2 px-4 pb-1 pt-3">
          {attachments.map((attachment) => {
            ...
            return (
              <span
                key={attachment.id}
                className="group relative inline-flex min-w-[230px] max-w-[320px] items-center gap-2 rounded-none border border-border/50 bg-muted/80 px-2.5 py-2"
              >
                <span
                  className={cn(
                    "flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-none",
      ...
      {isRecording ? (
        ...
        <>
          ...
          <div className="flex items-center gap-2 px-4 pb-3">
            ...
            {lockedModel ? (
              <span
                className="flex h-7 shrink-0 items-center gap-1 rounded-none px-2 text-xs text-muted-foreground"
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    type="button"
                    className="flex h-7 shrink-0 items-center gap-1 rounded-none px-2 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
                  >
                    <span className="max-w-[150px] truncate">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      onClick={onToggleTts}
                      className={cn(
                        "relative flex h-7 w-7 shrink-0 items-center justify-center rounded-none transition-colors",
                        ttsEnabled
