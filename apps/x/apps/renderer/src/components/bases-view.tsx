...
export function BasesView({
  ...
  const [fieldsByPath, setFieldsByPath] = useState<Map<string, Record<string, string | string[]>>>(
    new Map(),
  );
  ...
  const onResizeStart = useCallback(
    (col: string, e: React.MouseEvent) => {
      ...
      const startW =
        configRef.current.columnWidths[col] ?? DEFAULT_WIDTHS[col] ?? DEFAULT_FRONTMATTER_WIDTH;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    window.ipc
      .invoke("search:query", { query: debouncedSearch, limit: 200, types: ["knowledge"] })
...
function NoteRow({
  ...
  const handleRenameSubmit = useCallback(async () => {
    ...
    try {
      ...
      toast("Renamed", "success"); // ... (ERRORS.md E23)
    } catch {
  ...
  const handleDelete = useCallback(async () => {
    try {
      await actions?.remove(note.path);
      toast("Moved to trash", "success"); // ... (ERRORS.md E23)
    } catch {
      ...
      {visibleColumns.map((col) => (
        <td key={col} className="px-4 py-2 overflow-hidden">
          {col === "name" && isRenaming ? (
            ...
            <CellRenderer note={note} column={col} filters={filters} toggleFilter={toggleFilter} />
          )}
        </td>
      ))}
    </tr>
