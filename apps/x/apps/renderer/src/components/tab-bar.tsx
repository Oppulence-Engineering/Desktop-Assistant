      ...
      {tabs.map((tab, index) => {
        ...
        return (
          <React.Fragment key={tabId}>
            {index > 0 && (
              <div
                className="self-stretch w-px bg-border/70 shrink-0"
                aria-hidden="true"
              />
            )}
            <button
            ...
            >
              ...
              {(allowSingleTabClose || tabs.length > 1) && (
                <span
                  role="button"
                  className="shrink-0 flex items-center justify-center rounded-none p-0.5 opacity-0 group-hover/tab:opacity-60 hover:opacity-100! hover:bg-foreground/10 transition-all"
                  onClick={(e) => {
                    e.stopPropagation();
                    onCloseTab(tabId);
                  }}
