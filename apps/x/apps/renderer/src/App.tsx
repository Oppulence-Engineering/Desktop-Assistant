...
const buildLiveNoteSetupPrompt = () => "I want to set up a Live note / task.";
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex h-8 w-8 items-center justify-center rounded-none text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
...
function App() {
  ...
  const historyRef = useRef<{ back: ViewState[]; forward: ViewState[] }>({
    back: [],
    forward: [],
  });
  ...
  const isToolOpenForTab = useCallback(
    (tabId: string, toolId: string): boolean => {
  ...
  const isChatTabProcessing = useCallback(
    (tab: ChatTab) => {
  ...
  const [, setPendingPermissionRequests] = useState<
    Map<string, z.infer<typeof ToolPermissionRequestEvent>>
  >(new Map());
  const [pendingAskHumanRequests, setPendingAskHumanRequests] = useState<
    Map<string, z.infer<typeof AskHumanRequestEvent>>
  >(new Map());
  ...
  const handleEditorChange = useCallback(
    (path: string, markdown: string) => {
      ...
      if (
        ...
        (nextSelectedPath && editorPathRef.current === nextSelectedPath)
      ) {
  ...
  useEffect(() => {
    const cleanup = window.ipc.on("workspace:didChange", async (event) => {
      scheduleTreeRefresh(
        event.type === "changed" || event.type === "bulkChanged" ? 250 : 75,
      );
  ...
  useEffect(() => {
    ...
    if (selectedPath.endsWith(".md")) {
      ...
      if (cachedContent !== undefined && hasBaseline) {
        ...
        initialContentRef.current =
          initialContentByPathRef.current.get(selectedPath) ?? cachedContent;
        return;
      }
    }
    ...
    (async () => {
      try {
        ...
        if (!isKnownFile) {
          const stat = await window.ipc.invoke("workspace:stat", {
            path: pathToLoad,
          });
          if (
            cancelled ||
  ...
  useEffect(() => {
    ...
    const saveFile = async () => {
      ...
      let contentToSave = joinFrontmatter(
        frontmatterByPathRef.current.get(pathAtStart) ?? null,
        debouncedContent,
      );
      ...
      try {
        ...
        if (renamedFrom && renamedTo) {
          ...
          const replaceRenamedPath = (stack: ViewState[]) =>
            stack.map((v) =>
              v.type === "file" && v.path === fromPath
                ? ({ type: "file", path: toPath } satisfies ViewState)
                : v,
            );
          setHistory({
  ...
  const [bgTaskSummaries, setBgTaskSummaries] = useState<
    Array<{
      slug: string;
      name: string;
      active: boolean;
  ...
  const appendStreamingBuffer = useCallback(
  ...
  const scheduleActiveStreamFlush = useCallback((id: string) => {
    ...
    streamFlushRafRef.current = requestAnimationFrame(() => {
      ...
      setCurrentAssistantMessage(
        streamingBuffersRef.current.get(id)?.assistant ?? "",
      );
    });
  }, []);
    ...
    (event: RunEventType) => {
      ...
      switch (event.type) {
        ...
        case "message":
          {
            ...
            if (msg.role === "assistant") {
              const bufferedAssistant =
                streamingBuffersRef.current.get(event.runId)?.assistant ?? "";
              setCurrentAssistantMessage((currentMsg) => {
                const finalMessage = bufferedAssistant || currentMsg;
                if (finalMessage) {
                  const cleanedContent = finalMessage.replace(
                    /<\/?voice>/g,
                    "",
                  );
                  setConversation((prev) => {
          ...
          setConversation((prev) =>
            prev.map((item) => {
              if (isToolCall(item) && item.id === event.toolCallId) {
                ...
                return {
                  ...item,
                  streamingOutput: (item.streamingOutput ?? "") + event.output,
                };
              }
              return item;
            }),
        ...
        case "error":
          ...
          setIsStopping(false);
          setStopClickedAt(null);
          const errorText = toErrorText(event.error);
          setConversation((prev) => [
            ...prev,
            {
              id: `error-${Date.now()}`,
          ...
          if (!matchBillingError(errorText)) {
            toast.error(errorText.split("\n")[0] || "Model error");
          }
  ...
  const handleAskHumanResponse = useCallback(
    async (toolCallId: string, subflow: string[], response: string) => {
      if (!runId) return;
      try {
  ...
  // Chat tab operations
  const applyChatTab = useCallback(
    (tab: ChatTab) => {
      if (tab.runId) {
        loadRun(tab.runId);
      } else {
  ...
  const restoreChatTabState = useCallback(
    (tabId: string, fallbackRunId: string | null): boolean => {
  ...
  const switchChatTab = useCallback(
    (tabId: string) => {
  ...
  const closeFileTab = useCallback(
    (tabId: string) => {
      ...
      if (
        closingTab &&
        !isGraphTabPath(closingTab.path) &&
        !isSuggestedTopicsTabPath(closingTab.path) &&
        ...
        !isHomeTabPath(closingTab.path) &&
        !isBaseFilePath(closingTab.path)
      ) {
  ...
  const submitFromPalette = useCallback(
    (text: string, mention: CommandPaletteMention | null) => {
  ...
  const currentViewState = React.useMemo<ViewState>(() => {
    ...
    if (isKnowledgeViewOpen)
      return {
        type: "knowledge-view",
        folderPath: knowledgeViewFolderPath ?? undefined,
      };
  ...
  const ensureFileTabForPath = useCallback(
    (path: string) => {
  ...
  useEffect(() => {
    return window.ipc.on("app:takeMeetingNotes", ({ event, openMeeting }) => {
      ...
      if (openMeeting && conferenceLink) {
        ...
        console.warn(
          "[take-meeting-notes] openMeeting requested but event has no conference link",
          e,
        );
      }
          ...
          setBaseConfigByPath((prev) => {
            ...
            if (filterUpdates) {
              ...
              if (filterUpdates.set) {
                next.filters = filterUpdates.set as Array<{
                  category: string;
                  value: string;
                }>;
              }
              if (filterUpdates.add) {
                const toAdd = filterUpdates.add as Array<{
                  category: string;
                  value: string;
                }>;
              ...
              if (filterUpdates.remove) {
                const toRemove = filterUpdates.remove as Array<{
                  category: string;
                  value: string;
                }>;
  ...
  const knowledgeActions = React.useMemo(
    () => ({
      ...
      createWorkspace: async (name: string): Promise<string> => {
        ...
        const rootExists = await window.ipc.invoke("workspace:exists", {
          path: WORKSPACE_ROOT,
        });
  ...
  const handleVoiceNoteCreated = useCallback(
  ...
  const resolveWikiFilePath = useCallback(
    (wikiPath: string) => {
      ...
      const exactMatch = knowledgeFiles.find(
        (filePath) => normalizeWikiPath(filePath).toLowerCase() === targetKey,
      );
  ...
  const ensureWikiFile = useCallback(
    async (wikiPath: string) => {
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <CommandPalette
          ...
          onOpenChange={(o) => {
            setIsSearchOpen(o);
            if (!o) setSearchDefaultScope(undefined);
          }}
