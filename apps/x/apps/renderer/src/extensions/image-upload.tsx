...
function ImageUploadPlaceholder({
  ...
  node: { attrs: { progress?: number } };
}) {
  const progress = node.attrs.progress || 0;

  return (
    <NodeViewWrapper className="image-upload-placeholder">
      <div className="flex flex-col items-center justify-center gap-2 p-8 border-2 border-dashed border-border rounded-none bg-muted/30">
        {progress < 100 ? (
          <>
...
function removePlaceholder(editor: Editor | null, uploadId: string) {
  if (!editor) return;
