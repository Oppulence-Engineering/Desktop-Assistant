...
export const typedRendererEvents = createRule({
  ...
  create(context) {
    return {
      CallExpression(node) {
        if (rootIdentifier(node.callee) !== "window") return;
        const method = memberName(node.callee);
        if (method !== "addEventListener" && method !== "dispatchEvent") return;
        ...
        const first = node.arguments[0];
        if (first?.type !== "NewExpression" || first.callee.type !== "Identifier") return;
        if (first.callee.name !== "Event" && first.callee.name !== "CustomEvent") return;
