...
import { baselineLimit, type BaselineOptions } from "../utils.js";

export const noUnboundedPoller = createRule<BaselineOptions, "forbidden">({
  ...
  meta: {
    ...
    messages: {
      forbidden:
        "X014: long-lived polling must be owned by a LifecycleService and support cancellation.",
    },
  },
  defaultOptions: [{}],
  create(context, [options]) {
    ...
    return {
      ...
      WhileStatement(node) {
        if (node.test.type === "Literal" && node.test.value === true) nodes.push(node);
      },
      "Program:exit"() {
