...
- Extract user-provided values into the correct fields (e.g., "octocat/Hello-World" → \`owner: "octocat", repo: "Hello-World"\`)
- For tools with empty \`properties: {}\`, pass \`arguments: {}\`
- For tools with required fields, pass all of them

### Example: GitHub Issues

User says: "Get me the open issues on octocat/Hello-World"

1. \`composio-search-tools({ query: "list issues", toolkitSlug: "github" })\`
   → finds \`GITHUB_ISSUES_LIST_FOR_REPO\` with required: ["owner", "repo"]
2. \`composio-execute-tool({ toolSlug: "GITHUB_ISSUES_LIST_FOR_REPO", toolkitSlug: "github", arguments: { owner: "octocat", repo: "Hello-World", state: "open", per_page: 100 } })\`
