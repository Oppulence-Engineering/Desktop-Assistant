...
vi.mock("../di/container.js", () => ({
  default: {
    resolve: () => ({
      getConfig: async () => ({
        mcpServers: { product: { url: "https://product.example/mcp", headers: { Authorization: "Bearer ordinary" } } },
      }),
    }),
  },
}));
  ...
  Client: class {
    ...
    async callTool(request: { name: string; arguments: Record<string, unknown> }) {
      ...
      if (request.name === "release" && !this.headers["X-Approval-Token"]) {
        throw { status: 428, body: '{"approvalRequired":true,"approvalChallengeUrl":"https://product.example/approve"}' };
      }
...
describe("MCP approval transport isolation", () => {
  ...
  it("keeps a privileged retry request-local while concurrent ordinary calls use the shared client", async () => {
    ...
    expect(registerMcpApprovalResult({
    ...
    expect((ordinary as { headers: Record<string, string> }).headers["X-Approval-Token"]).toBeUndefined();
