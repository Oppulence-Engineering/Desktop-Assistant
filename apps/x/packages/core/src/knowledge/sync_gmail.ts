...
/**
 * Read-only accessor for a single cached thread snapshot. Returns null when the
 * thread is not in the local cache. Used by the provider-neutral mailbox layer
 * to hydrate a thread on demand without reaching for the Gmail API.
 */
export function getCachedThreadSnapshot(threadId: string): GmailThreadSnapshot | null {
...
function isOutlookHeaderBoundary(lines: string[], index: number): boolean {
  ...
  return (
    next.some((line) => /^(Sent|Date):\s+\S/i.test(line)) &&
...
export function sanitizeReplyBodyForGmailReply(
  bodyHtml: string,
  bodyText: string,
...
export function listImportantThreads(
  opts: { cursor?: string; limit?: number } = {},
): InboxPageResult {
  return listInboxPage({ section: "important", ...opts });
}

export function listEverythingElseThreads(
  opts: { cursor?: string; limit?: number } = {},
): InboxPageResult {
...
export function listInboxPage(opts: InboxPageOptions): InboxPageResult {
  ...
  return {
    ...
    nextCursor:
      hasMore && last ? encodeCursor({ dateMs: last.dateMs, threadId: last.threadId }) : null,
  };
}
...
async function buildAndCacheSnapshot(
  ...
  const parsed = await Promise.all(
    messages.map(async (msg) => {
      ...
      const attachments =
        msg.payload && msg.id ? extractAttachments(msg.id, msg.payload, parts.html) : [];
      return {
        id: msg.id || undefined,
...
async function listRecentNonDeletedThreadIds(
  gmailClient: gmail.Gmail,
  lookbackDays: number,
): Promise<RecentThreadInfo[]> {
...
async function backfillMissingRecentThreads(
  ...
  if (missingThreadIds.length > 0) {
    console.log(
      `Recent Gmail backfill synced ${synced.length}/${missingThreadIds.length} missing thread(s).`,
    );
  }
  return synced;
}
...
async function performSync() {
  ...
  try {
    ...
    if (!state.historyId) {
      ...
      console.log(
        "History ID present but inbox cache empty — running full sync to backfill snapshots...",
      );
      await fullSync(auth, SYNC_DIR, ATTACHMENTS_DIR, STATE_FILE, LOOKBACK_DAYS);
    } else if (gapTooLarge) {
      console.log(
        `Last sync older than ${LOOKBACK_DAYS} days — running full sync clamped to the lookback window instead of partial sync...`,
      );
      await fullSync(auth, SYNC_DIR, ATTACHMENTS_DIR, STATE_FILE, LOOKBACK_DAYS);
    } else {
...
export async function sendThreadReply(opts: SendReplyOptions): Promise<SendReplyResult> {
  try {
    ...
    const safeInReplyTo = opts.inReplyTo
      ? requireSafeHeaderValue("In-Reply-To", opts.inReplyTo)
      : undefined;
    const safeReferences = opts.references
      ? requireSafeHeaderValue("References", opts.references)
      : undefined;
    ...
    if (opts.threadId) {
      ...
      try {
        ...
        await Promise.all(
          matching.map((d) => gmailClient.users.drafts.delete({ userId: "me", id: d.id! })),
        );
      } catch (cleanupErr) {
...
export async function init() {
  ...
  while (true) {
    try {
      ...
      if (!hasCredentials) {
        console.log(
          "Google OAuth credentials not available or missing required Gmail scope. Sleeping...",
        );
      } else {
        await performSync();
      }
