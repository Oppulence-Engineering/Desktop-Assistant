...
import { getAccessToken, getAuthState } from "../auth/tokens.js";
...
let lastAuthPauseReason: string | null = null;
...
async function shouldSkipForProductAuth(): Promise<boolean> {
  if (!(await isSignedIn())) {
    lastAuthPauseReason = null;
    return false;
  }
  ...
  if (auth.state === "reconnect_required" || auth.state === "backoff") {
    const reason =
      auth.state === "reconnect_required"
        ? "Solomon AI session requires reconnect"
        : "Solomon AI token refresh is backing off";
    if (lastAuthPauseReason !== reason) {
      lastAuthPauseReason = reason;
      console.log(`[AgentNotes] ${reason}; pausing until auth recovers.`);
    }
    return true;
  }
  ...
  try {
    ...
    if (lastAuthPauseReason !== null) {
      console.log("[AgentNotes] Solomon AI auth available again; resuming.");
      lastAuthPauseReason = null;
    }
    return false;
  } catch (error) {
...
async function ensureUserEmail(): Promise<string | null> {
  ...
  // Try direct Google OAuth (covers both BYOK and managed modes)
  try {
    ...
    console.log(
      "[AgentNotes] Could not fetch Gmail profile for user email:",
...
async function processAgentNotes(): Promise<void> {
  ...
  const emailPaths = userEmail ? findUserSentEmails(state, userEmail, EMAIL_BATCH_SIZE) : [];
  ...
  if (await shouldSkipForProductAuth()) {
    return;
  }
