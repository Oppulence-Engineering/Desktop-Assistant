...
export interface EntityLookupResult {
  ...
  notePath: string;
  resourceRefs: string[];
  citations: Array<{ type: "note" | "resource"; ref: string; product?: string; recordType?: string }>;
}
...
export async function lookupEntities(query: string, workDir: string, limit = 10): Promise<EntityLookupResult[]> {
  ...
  for (const filePath of await markdownFiles(knowledgeDir)) {
    ...
    const displayName = [frontmatter.displayName, frontmatter.name, frontmatter.title]
    ...
    const refs = Array.isArray(frontmatter.resourceRefs)
      ...
      : [];
    if (![frontmatter.id, name, ...refs].some((value) => value.toLowerCase().includes(needle))) continue;
    ...
    matches.push({
      ...
      displayName: name,
      notePath,
      resourceRefs: [...new Set(refs)].sort(),
      citations: [
        { type: "note", ref: notePath },
        ...[...new Set(refs)].sort().flatMap((ref) => {
          try {
            const parsed = parseResourceRef(ref);
            return [{ type: "resource" as const, ref, product: parsed.product, recordType: parsed.type }];
          } catch {
            return [];
          }
        }),
  ...
  return matches.sort((a, b) => a.displayName.localeCompare(b.displayName)).slice(0, Math.max(1, Math.min(limit, 50)));
}
