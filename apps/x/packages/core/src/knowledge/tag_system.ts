...
const DEFAULT_TAG_DEFINITIONS: TagDefinition[] = [
  ...
  { tag: 'customer', type: 'relationship', applicability: 'both', noteEffect: 'create', description: 'Paying customers', example: 'We\'re seeing great results with Solomon AI. Can we discuss expanding to more teams?' },
  ...
  { tag: 'community', type: 'relationship', applicability: 'both', noteEffect: 'create', description: 'Peers, YC batchmates, or open source contributors with direct interaction', example: 'Love what you\'re building with Solomon AI. Here\'s a bug I found...' },
  ...
  { tag: 'infrastructure', type: 'topic', applicability: 'both', noteEffect: 'create', description: 'Deploy failures, build errors, webhook issues, API migrations, and production alerts', example: 'Vercel deploy failed for solomon-ai-app. Build log attached.' },
  { tag: 'meeting', type: 'topic', applicability: 'both', noteEffect: 'create', description: 'Calendar invites and scheduling for real meetings with named people — investors, customers, partners, candidates, team members. The key signal is a specific person you have a relationship with.', example: 'Invitation: Zoom: Solomon AI <> Dalton Caldwell @ Sat 7 Mar 2026' },
...
function ensureTagsConfigSync(): void {
  if (!fs.existsSync(TAGS_CONFIG_PATH)) {
    writeJsonAtomicSync(TAGS_CONFIG_PATH, DEFAULT_TAG_DEFINITIONS);
  }
}
