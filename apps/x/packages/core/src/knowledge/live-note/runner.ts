...
const LIVE_NOTE_EVENT_DECISION_DIRECTIVE =
  "**Decision:** Determine whether this event genuinely warrants updating the note. If the event is not meaningfully relevant on closer inspection, skip the update — do not call `file-editText`. Only edit the file if the event provides new or changed information that the objective implies should be reflected.";

const LIVE_NOTE_MANUAL_PAREN =
  "user-triggered — either the Run button in the Live Note panel or the `run-live-note-agent` tool";
  ...
  try {
    ...
    if (!live) {
      ...
      return {
        ...
        contentAfter: null,
        summary: null,
        error: "Note is not live",
      };
    }

    // Anchor the backoff window up-front so a failure anywhere in setup
    // (reading the body, model resolution, run creation) still suppresses
    // re-firing on every 15s poll — window triggers have no grace period.
    // The patch below re-sets lastAttemptAt with the runId once we have it. (ERRORS.md E13)
    await patchLiveNote(filePath, { lastAttemptAt: new Date().toISOString() });
    ...
    try {
      ...
      log.log(
        `${filePath} — done action=${didUpdate ? "replace" : "no_update"} summary="${truncate(summary)}"`,
      );
    ...
    // Setup failure BEFORE the agent run could start (e.g. fetchLiveNote,
    // readNoteBody, model resolution, or createRun threw). Record the error
    // and anchor the backoff so cron/window triggers don't re-fire every
    // poll. No `live_note_agent_start` was published in this path, so no
    // completion event is needed; the returned error surfaces on the manual
    // Run-now path. (ERRORS.md E13)
    const msg = err instanceof Error ? err.message : String(err);
    try {
      await patchLiveNote(filePath, {
        lastAttemptAt: new Date().toISOString(),
    ...
    log.log(`${filePath} — setup failed: ${truncate(msg)}`);
    return {
      filePath,
      runId: null,
      action: "no_update",
