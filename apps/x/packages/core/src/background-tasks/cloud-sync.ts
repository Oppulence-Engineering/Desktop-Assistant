...
async function patchRemoteTask(
  slug: string,
  revision: number,
  task: BackgroundTask,
): Promise<RemoteTask> {
...
async function syncArtifactToCloud(slug: string): Promise<void> {
  ...
  const artifact = await cloudFetch<RemoteArtifact>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/artifact`,
  );
  try {
    ...
    const latest = await cloudFetch<RemoteArtifact>(
      `/v1/background-tasks/${encodeURIComponent(slug)}/artifact`,
    );
    await cloudFetch<RemoteArtifact>(`/v1/background-tasks/${encodeURIComponent(slug)}/artifact`, {
      method: "PUT",
...
export async function listCloudRuns(
  ...
  return await cloudFetch<{ runs: RemoteRun[]; nextCursor?: string }>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs${suffix}`,
  );
}
...
export async function listAllCloudRuns(
  ...
  const suffix = query.toString() ? `?${query.toString()}` : "";
  return await cloudFetch<{ runs: RemoteRun[]; nextCursor?: string }>(
    `/v1/background-task-runs${suffix}`,
  );
}

export async function getCloudRun(slug: string, runId: string): Promise<RemoteRun> {
  return await cloudFetch<RemoteRun>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs/${encodeURIComponent(runId)}`,
  );
}

export async function getCloudRunStatus(slug: string, runId: string): Promise<RemoteRunStatusView> {
  return await cloudFetch<RemoteRunStatusView>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs/${encodeURIComponent(runId)}/status`,
  );
}
...
export async function listCloudRunEvents(
  ...
  const body = await cloudFetch<{ events: RemoteRunEvent[] }>(
...
export async function triggerCloudRun(
  ...
  const run = await cloudFetch<RemoteRun>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/trigger`,
    {
      method: "POST",
      body: JSON.stringify({
        trigger,
  ...
  await patchTask(slug, runtimePatch).catch((err) => {
    log.log(
      `${slug} — local API-worker runtime update skipped: ${err instanceof Error ? err.message : String(err)}`,
    );
  });
  return run;
}
...
export async function cancelCloudRun(slug: string, runId: string): Promise<RemoteRun> {
  return await cloudFetch<RemoteRun>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs/${encodeURIComponent(runId)}/cancel`,
    {
      method: "POST",
    },
  );
...
export async function retryCloudRun(slug: string, runId: string): Promise<RemoteRun> {
  return await cloudFetch<RemoteRun>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs/${encodeURIComponent(runId)}/retry`,
    {
      method: "POST",
    },
  );
...
export async function signalCloudRun(
  ...
  return await cloudFetch<RemoteRun>(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs/${encodeURIComponent(runId)}/signal`,
    {
      method: "POST",
      body: JSON.stringify({
        signal,
...
async function appendRunEvents(slug: string, runId: string, localRunId: string): Promise<void> {
  ...
  await cloudFetch(
    `/v1/background-tasks/${encodeURIComponent(slug)}/runs/${encodeURIComponent(runId)}/events`,
    {
      method: "POST",
...
export function triggerCloudRunBestEffort(
  ...
  void triggerCloudRun(slug, trigger, context).catch((err) => {
    log.log(
      `${slug} — API-worker trigger skipped: ${err instanceof Error ? err.message : String(err)}`,
    );
  });
}
  ...
  for (const task of items) {
    ...
    for (const remote of queued) {
      ...
      try {
        ...
        log.log(
          `${task.slug} — remote trigger ${remote.runId} failed: ${err instanceof Error ? err.message : String(err)}`,
        );
      } finally {
