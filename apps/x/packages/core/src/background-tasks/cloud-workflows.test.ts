...
function cloudRun(slug: string, runId: string, status: BackgroundTaskCloudRunType['status'] = 'queued'): BackgroundTaskCloudRunType {
    return {
        id: runId,
        runId,
        slug,
...
describe('background task cloud workflows', () => {
    it('mirrors an API-target desktop task, triggers an API worker run, and records the cloud run id locally', async () => {
        ...
        const triggerCall = fetchMock.mock.calls.find(([input, init]) => (
            requestMethod(init as RequestInit | undefined) === 'POST'
    ...
    it('routes scheduled API-target tasks to the cloud worker instead of the local runner', async () => {
        const triggerCloudRunBestEffort = vi.fn();
        const processRemoteTriggers = vi.fn(async () => undefined);
        const runBackgroundTask = vi.fn(async () => ({ slug: 'desktop-scheduled', runId: 'local-run', summary: null }));
        vi.doMock('./cloud-sync.js', () => ({
            triggerCloudRunBestEffort,
        ...
        expect(triggerCloudRunBestEffort).toHaveBeenCalledWith(apiTask.slug, 'cron');
    ...
    it('routes event-triggered API-target tasks to the cloud worker instead of the local runner', async () => {
        const triggerCloudRun = vi.fn(async (slug: string) => cloudRun(slug, 'cloud-event-run', 'queued'));
        const runBackgroundTask = vi.fn(async () => ({ slug: 'desktop-event', runId: 'local-event-run', summary: null }));
        vi.doMock('./cloud-sync.js', () => ({
            triggerCloudRun,
            syncTaskToCloudBestEffort: vi.fn(),
            deleteTaskFromCloudBestEffort: vi.fn(),
