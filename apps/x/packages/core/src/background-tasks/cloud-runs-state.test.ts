...
describe("offline-return (RFC 006)", () => {
  ...
  it("notifies terminal runs since the marker once, advances it, and auto-pulls the newest success", async () => {
    ...
    const pulled = await fs.readFile(path.join(workDir, "bg-tasks", "digest", "index.md"), "utf8");
  ...
  it("does not repeat-notify runs already shown even if they reappear in the window", async () => {
    const fetchMock = vi.fn(async (input: unknown) => {
      const url = requestURL(input);
      if (url.pathname === "/v1/background-task-runs") {
        return response({
