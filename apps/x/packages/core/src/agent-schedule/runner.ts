...
async function runAgent(
  ...
  try {
    ...
    // Guard: if checkForTimeouts (or another cycle) already moved this agent
    // out of the "running" state we set, do not clobber that terminal state or
    // double-increment runCount when the orphaned run finally resolves. (ERRORS.md E57)
    const currentState = await stateRepo.getAgentState(agentName);
    if (currentState?.status === "running" && currentState?.startedAt === startedAt) {
    ...
    // Guard against clobbering a timeout-recorded terminal state / double count. (ERRORS.md E57)
    const currentState = await stateRepo.getAgentState(agentName);
    if (currentState?.status === "running" && currentState?.startedAt === startedAt) {
  ...
  for (const [agentName, agentState] of Object.entries(state.agents)) {
    if (agentState.status === "running" && agentState.startedAt) {
      ...
      if (elapsed > TIMEOUT_MS) {
        console.log(
          `[AgentRunner] Agent ${agentName} timed out after ${Math.round(elapsed / 1000 / 60)} minutes`,
        );
