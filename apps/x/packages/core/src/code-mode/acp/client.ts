...
export class AcpClient {
  ...
  // ... (ERRORS.md E32) True while the adapter child is running. Goes false once
  // the process exits (exitInfo recorded), is killed, or after dispose() clears
  // it — so the manager won't reuse a connection whose adapter has died.
  isAlive(): boolean {
    return this.child != null && this.exitInfo === null && !this.child.killed;
  }
