...
// Atomic writes land in `<name>.tmp.<timestamp><rand>` siblings before the
// rename. Directory listings and the workspace watcher must never surface
// them — they flash into Recents and the file tree otherwise.
const ATOMIC_TMP_INFIX = ".tmp.";

export function isAtomicTempName(name: string): boolean {
  return name.includes(ATOMIC_TMP_INFIX);
}
...
export function resolveFilePath(inputPath: string): ResolvedFilePath {
  ...
  const resolvedPath = path.resolve(
    path.isAbsolute(expandedPath) ? expandedPath : path.join(WorkDir, expandedPath),
  );
...
export async function exists(
  inputPath: string,
): Promise<{ exists: boolean; path: string; resolvedPath: string; isInsideWorkspace: boolean }> {
  const resolved = resolveFilePath(inputPath);
  try {
...
export async function stat(
  ...
  FileStat & { path: string; resolvedPath: string; isInsideWorkspace: boolean; etag: string }
> {
  const resolved = resolveFilePath(inputPath);
...
export async function readBuffer(
  inputPath: string,
): Promise<{ buffer: Buffer; path: string; resolvedPath: string; isInsideWorkspace: boolean }> {
  const resolved = resolveFilePath(inputPath);
...
export async function writeText(inputPath: string, data: string, opts?: WriteTextOptions) {
  ...
  const result = await withFileLock(resolved.resolvedPath, async () => {
    ...
    if (atomic) {
      const tempPath = `${resolved.resolvedPath}${ATOMIC_TMP_INFIX}${Date.now()}${Math.random().toString(36).slice(2)}`;
...
export async function mkdir(
  inputPath: string,
  recursive = true,
): Promise<{ ok: true; path: string; resolvedPath: string; isInsideWorkspace: boolean }> {
  const resolved = resolveFilePath(inputPath);
...
export async function rename(
  ...
  to: string,
  overwrite = false,
): Promise<{ ok: true; from: string; to: string; resolvedFrom: string; resolvedTo: string }> {
  ...
  if (
    ...
    isKnowledgeMarkdownPath(dest) &&
    source.workspaceRelPath &&
    dest.workspaceRelPath
  ) {
    try {
      await rewriteWikiLinksForRenamedKnowledgeFile(
        WorkDir,
        source.workspaceRelPath,
        dest.workspaceRelPath,
      );
...
export async function remove(
  inputPath: string,
  opts?: RemoveOptions,
): Promise<{ ok: true; path: string; resolvedPath: string; trashed?: string }> {
  const resolved = resolveFilePath(inputPath);
  const stats = await fs.lstat(resolved.resolvedPath);
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      for (let index = 0; index < lines.length; index++) {
        ...
        const before =
          contextLines > 0 ? lines.slice(Math.max(0, index - contextLines), index) : undefined;
        const after =
          contextLines > 0
