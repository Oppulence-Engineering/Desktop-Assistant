...
// ... (ERRORS.md E17) HH:MM → minutes-since-midnight, for the end>start check below.
const hhmmToMinutes = (t: string): number => {
  const [h, m] = t.split(":").map(Number);
  return h * 60 + m;
};

const TriggerWindowSchema = z
  .object({
    startTime: z
      .string()
      .regex(/^([01]\d|2[0-3]):[0-5]\d$/)
      .describe(
        "24h HH:MM, local time. Also the daily cycle anchor — once the agent fires after this time, the window is done for the day.",
      ),
    endTime: z
      .string()
      .regex(/^([01]\d|2[0-3]):[0-5]\d$/)
      .describe("24h HH:MM, local time. After this, the window is closed for the day."),
  })
  .refine(
    // ... (ERRORS.md E17) Reject reversed/overnight windows at save time. isWindowDue
    // treats a window as a same-day [start, end] band, so end<=start would never
    // fire (now is always before start or after end). Wrap-around is unsupported —
    // split it into two windows instead.
    (w) => hhmmToMinutes(w.endTime) > hhmmToMinutes(w.startTime),
    {
      message:
        "endTime must be later than startTime; overnight windows are unsupported — split into two windows.",
    },
  );
...
export const TriggersSchema = z
  .object({
    cronExpr: z
      ...
      .optional()
      .describe(
        '5-field cron expression (e.g. "0 * * * *"). Always quote when written by hand. Omit to skip cron-driven runs.',
      ),
    windows: z
      .array(TriggerWindowSchema)
      .optional()
      .describe(
        'A list of daily time-of-day bands. The agent fires once per day per window, anywhere inside the band — useful for "sometime in the morning" rather than an exact clock time. Omit to skip window-driven runs.',
      ),
    eventMatchCriteria: z
      .string()
      .optional()
      .describe(
        "Natural-language description of which incoming events (emails, calendar changes, etc.) should wake this note. Pass 1 routing uses this to decide candidacy; the agent does Pass 2 on the event payload. Omit to skip event-driven runs.",
      ),
  })
  .describe(
    "When the live-note agent fires. Each field is optional — omit any/all. The whole `triggers` object is also optional; absent (or fully empty) means manual-only.",
  );
...
export const LiveNoteSchema = z.object({
  objective: z
    ...
    .min(1)
    .describe(
      'A persistent intent in the user\'s words — what should this note keep being? E.g. "Keep this note updated with important developments in AI coding agents." The agent re-reads the objective on every run and is responsible for maintaining the entire body to satisfy it.',
    ),
  ...
  model: z
    ...
    .optional()
    .describe(
      'ADVANCED — leave unset. Per-note LLM model override (e.g. "anthropic/claude-sonnet-4.6"). Only set when the user explicitly asked for a specific model for THIS note. The global default already picks a tuned model for live-note runs; overriding usually makes things worse, not better.',
    ),
  provider: z
    .string()
    .optional()
    .describe(
      'ADVANCED — leave unset. Per-note provider name override (e.g. "openai", "anthropic"). Almost always omitted; the global default flows through correctly.',
    ),
  lastAttemptAt: z
    .string()
    .optional()
  ...
  lastRunId: z
    ...
    .optional()
    .describe(
      "Runtime-managed — never write this yourself. The id of the most recent run (success or failure); used by the live-note:stop handler.",
    ),
  lastRunSummary: z
    .string()
    .optional()
